...
## Explicit source profiles and the six-hour candidate run
...
```sh
python3 -B tests/ft1536/dudect/prepare.py --repo "$PWD" --profile floor-ct --defer-timing --seconds 21600 --work /home/footfalcon/free_falcon_sign/proofs/ft1536/work/FT1536_FPEMU_DUDECT_RUN_002
```
...
The budget is recorded as global_seconds; the old default/maximum is28800,
while the owner-requested RUN_002 campaign uses **21600 seconds (six hours)**.

After all proof workers/compilers have stopped, the explicit launch
command is:

```sh
python3 -B tests/ft1536/dudect/launch.py /home/footfalcon/free_falcon_sign/proofs/ft1536/work/FT1536_FPEMU_DUDECT_RUN_002 --seconds 21600
```
...
controls, with the prepared six-hour global campaign budget; launch preflight
...
reserve, and records source/commit/service identity. This setup does not
...
`campaign.py WORK --cpu CPU --seconds 21600` has three prespecified rounds,
...
not six hours of timed instructions alone. Finalization has a15s reserve.
Complete raw/statistics output is rate-limited to2MiB/s (at most about56.25GiB
over eight hours, or42.1875GiB over six hours, plus small metadata), by waiting between batches. This
...
equal to the prepared budget (21600 for this run), a bounded stop grace and KillMode=control-group, wrapping
