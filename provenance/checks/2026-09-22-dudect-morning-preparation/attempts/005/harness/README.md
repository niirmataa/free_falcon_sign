# FT1536 FPEMU — bounded official dudect campaigns

## Explicit source profiles and the ten-hour candidate run

The default `baseline` profile preserves the source pin below. The
`floor-ct` profile selects the independently reviewed, archived candidate
manifest **56974571b46e8257bdd3b4097c8c70fded6bb4b94c64805f6e35ec80929a0985**,
from `proofs/ft1536/stages/FT1536_FPEMU_FLOOR_CT_RUN_001/candidate/source`.
The default Extra/c build now has exactly those candidate bytes. The historical
baseline is retained under the FLOOR_CT archive's baseline/source and its
inputs/bootstrap/CANDIDATE.sha256, so it remains2553358f… after integration.
Both profiles use
the same official engine, benchmark.c, targets.c, fixtures and thresholds.

To prepare the candidate during daytime work, without physical timings:

```sh
python3 -B tests/ft1536/dudect/prepare.py --repo "$PWD" --profile floor-ct --defer-timing --seconds 36000 --work /home/footfalcon/free_falcon_sign/proofs/ft1536/work/FT1536_FPEMU_DUDECT_RUN_002
```

The resulting status is **STATIC_READY_TIMING_DEFERRED**, not PREFLIGHT_PASS
or RUNNING. Source pins, build, all28672 fixture checks, target assembly and
truncated-record checks run now. The pinned vendor can be read from the
existing archived preflight, avoiding a download. Every preparation attempt
retains its own PREPARATION.json.

For an existing unstarted WORK, add `--resume` to retain prior attempts and
create a new preparation. This is required after the source-profile routing,
duration or current harness changes. Never rewrite an old frozen preparation.
The budget is recorded as global_seconds. The default remains28800 seconds;
the maximum is36000. On2026-09-22 the owner extended the unstarted RUN_002
campaign to **36000 seconds (ten hours)**. The updated owner plan is a manual
morning start before leaving for work, after the owner's explicit signal.
This supersedes the earlier03:00 plan. Earlier preparations remain archived.

After all proof workers/compilers have stopped, the explicit launch
command is:
...
The launcher first performs a bounded fresh preparation/preflight (≤180s,
with5s kill grace), chooses a current non-SMT CPU and checks both controls.
For floor-ct, all three floor contrasts must have no signal and≥10^6 samples
per class in this short preflight. The baseline profile instead requires its
known floor signal. All raw preflight statistics are recalculated exactly.
A failure is recorded and does not launch the long campaign or silently retry.
Direct campaign invocation also rejects a deferred/not-passed preparation.

The long candidate campaign uses **ft1536-dudect-run-002.service**, separate
from baseline run001. The full schedule remains12 contrasts×3 rounds plus
controls, with the prepared ten-hour global campaign budget; launch preflight
is separate. The launcher forwards the same budget into the fresh preflight,
controller argv and systemd RuntimeMaxSec. Requested/prepared/refreshed budget
mismatches stop before service launch; direct campaign execution also requires
the prepared budget. The selected profile, source pin and actual budget appear
in LAUNCH/RUN/RESULT/report.
The launcher requires enough free disk for the prescribed log budget plus
reserve: **93751083008 bytes (87.3125GiB) for ten hours**, including70.3125GiB
of logs and17GiB reserve. Static build readiness does not certify this live
disk condition; the launcher checks it before physical preflight.
It records source/commit/service identity. This setup does not
schedule an automatic daytime or clock-time start.

After completion ACTIVE.json is explicitly FINISHED; RUN/RESULT contain the
actual campaign outcome. The default baseline has unit run001 as before.

## Baseline protocol and common implementation

Public scalar-only harness by Niirmata. The historical baseline17-file manifest is
...
The work's `PREPARATION.json` names the selected profile, frozen harness, binary, all hashes,
...
The baseline target must retain its known branch; the floor-ct target must
have no jump/call/loop/integer-division instructions. The broader original-TU
candidate review remains in the separately accepted FLOOR_CT checkpoint.
...
## Budgeted schedule and termination

`campaign.py WORK --cpu CPU --seconds 36000` has three prespecified rounds,
...
not ten hours of timed instructions alone. Finalization has a15s reserve.
Complete raw/statistics output is rate-limited to2MiB/s (at most70.3125GiB
over ten hours,56.25GiB over eight hours, or42.1875GiB over six hours, plus
small metadata), by waiting between batches. This
...
equal to the prepared budget (36000 for this run), a bounded stop grace and KillMode=control-group, wrapping
