...
DEFAULT_CAMPAIGN_SECONDS = 28800
MAX_CAMPAIGN_SECONDS = 36000


def campaign_seconds(value):
    if type(value) is not int or not 0 < value <= MAX_CAMPAIGN_SECONDS:
        raise RuntimeError(f'Campaign budget must be integer seconds in 1..{MAX_CAMPAIGN_SECONDS}')
    return value


def required_disk_bytes(seconds):
    return campaign_seconds(seconds) * LOG_BYTES_PER_SECOND + 17 * 2**30
...
def run_campaign(work, seconds, cpu):
    ...
    if prep['status'] != 'PREFLIGHT_PASS':
        raise RuntimeError('Physical timing preflight is required before campaign start')
    seconds = campaign_seconds(seconds)
    if cpu != prep['cpu'] or seconds != campaign_seconds(prep['global_seconds']):
    ...
    run.update(profile=prep.get('profile','baseline'),source_manifest_sha256=prep['source_manifest_sha256'])
    ...
    run['interpretation'] = ('EXPLORATORY_SHARED_HOST: inspect controls and all replications; '
                             'NO_LEAKAGE_EVIDENCE_YET is not CT proof; source profile is explicitly pinned')
    dump(work/'RUN.json', run)
    dump(work/'RESULT.json', run)
    dump(work/'ACTIVE.json',dict(status='FINISHED',campaign_status=run['status'],end_utc=run['end_utc']))
    dump(work/'machine_final.json', snapshot())
    with (work/'REPORT.md').open('w') as f:
        f.write('# FT1536 '+run['profile']+' dudect — campaign result\n\n')
        f.write('Source manifest: `'+run['source_manifest_sha256']+'`.\n\n')
        f.write(f"Status: **{run['status']}**; start {run['start_utc']}; end {run['end_utc']}.\n\n")
        f.write(f"Campaign budget: {run['budget_seconds']} seconds; elapsed {run['elapsed_seconds']:.3f} seconds.\n\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seconds', type=int, default=DEFAULT_CAMPAIGN_SECONDS)
