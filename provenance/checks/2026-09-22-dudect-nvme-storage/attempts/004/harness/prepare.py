...
from campaign import (CASES, DEFAULT_CAMPAIGN_SECONDS, MAX_CAMPAIGN_SECONDS, campaign_seconds, child_environment,
...
def prepare(repo, work, resume, profile_name='baseline', defer_timing=False,
            seconds=DEFAULT_CAMPAIGN_SECONDS):
    if not __debug__:
...
if __name__=='__main__':
    ...
    p.add_argument('--seconds',type=int,default=DEFAULT_CAMPAIGN_SECONDS,
                   help=f'Campaign budget, integer seconds up to{MAX_CAMPAIGN_SECONDS}; default{DEFAULT_CAMPAIGN_SECONDS}')
    a=p.parse_args()
