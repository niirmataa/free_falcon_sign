"""Pin, build, and preflight the baseline campaign, preserving every attempt."""
import argparse
import hashlib
import json
import math
...
from campaign import (CASES, child_environment, choose_cpu, dump, order_for,
                      replay_check, run_trial, sha, snapshot, utc)
...
SOURCE_PIN = '2553358fbb1144acdb577e1ad371a017ac325306dd901af745295dd17b03bd5a'
MASK = (1 << 64)-1
...
def prepare(repo, work, resume):
    ...
    manifest = repo/'provenance/ft1536-candidate.sha256'
    if sha(manifest) != SOURCE_PIN: raise RuntimeError('Source manifest pin')
    source = work/'source'; source.mkdir(exist_ok=True)
    for line in manifest.read_text().splitlines():
        digest, name = line.split()
        original = repo/'Extra/c'/name
    ...
    for name, blob in BLOBS.items():
        ...
        if actual != blob: raise RuntimeError('Upstream Git blob pin: '+name)
    ...
    # Bind the existing branch also to an original production TU, without running it.
    ...
    if not re.search(r'\bjs\b',target_asm): raise RuntimeError('Expected baseline floor branch absent: inspect build')
    ...
    for c in (1,0):
        ...
        row = run_trial(work,binary,folder,c,order_for(-1,c),30,cpu,batch_limit=22 if c==0 else 0)
        if not row['raw_complete'] or (c==1 and row['status']!='LEAKAGE_FOUND'):
            raise RuntimeError('Preflight positive/raw failure; all output retained')
        if c==0 and (row['status']!='NO_LEAKAGE_EVIDENCE_YET' or min(row['result']['n'])<1000000):
            raise RuntimeError('Preflight negative control failed; inspect retained measurements')
        checks.append(dict(case=CASES[c],status=row['status'],n=row['result']['n'],
                           raw_replay=replay_check(binary,folder)))
    # Real target callback + scalar floor branch, with a short bounded raw replay.
    folder=attempt/'preflight-floor'
    row=run_trial(work,binary,folder,2,order_for(-1,2),10,cpu,batch_limit=4)
    if not row['raw_complete']: raise RuntimeError('Floor smoke execution failure')
    checks.append(dict(case=CASES[2],status=row['status'],raw_replay=replay_check(binary,folder)))
    # Exercise normal wall-time termination and verify streaming receipt hashes.
    folder=attempt/'preflight-timebox'
    row=run_trial(work,binary,folder,0,order_for(-2,0),0.5,cpu)
    if not row['raw_complete'] or row['seconds']>3.5: raise RuntimeError('Timebox/cleanup failure')
    ...
    info=dict(status='PREFLIGHT_PASS',utc=utc(),cpu=cpu,binary=str(binary.relative_to(work)),
              harness=str(code.relative_to(work)),baseline_git_commit=job('baseline-head',['git','-C',repo,'rev-parse','HEAD']).strip(),
              source_manifest_sha256=SOURCE_PIN,upstream=origins,cflags=flags,checks=checks,sealed_files=sealed,
              measurement_profile='EXPLORATORY_SHARED_HOST; affinity, recorded governor/turbo; no exclusive CPU claim',
              case_names=CASES,rounds=3,batch_size=100000,global_seconds=28800,
              log_rate_limit_bytes_per_second=2*2**20)
    dump(work/'PREPARATION.json',info)
    print(json.dumps({k:info[k] for k in ('status','cpu','binary','harness','checks')},indent=2))
...
if __name__=='__main__':
    ...
    a=p.parse_args()
    prepare(a.repo.resolve(),a.work.resolve(),a.resume)
