...
def run_trial(work, binary, folder, case_id, order, seconds, cpu, batch_limit=0):
    ...
    with (folder/'stdout.txt').open('wb') as out, (folder/'stderr.txt').open('wb') as err:
        ...
        buffer = bytearray()
        eof = False
        compressed = gzip.open(folder/'timings.bin.gz', 'wb', compresslevel=1)
        stdout_reader = (folder/'stdout.txt').open()
        try:
            def receive(n):
                nonlocal eof
            ...
            while True:
                ...
                for line in stdout_reader:
                    if line.startswith('FT1536_BATCH '):
                        last_batch = json.loads(line.split(' ', 1)[1])
    ...
    for line in (folder/'stdout.txt').read_text().splitlines():
        if line.startswith('FT1536_RESULT '):
    ...
    row = dict(status=status, argv=argv, cpu=cpu, case_id=case_id, case=CASES[case_id],
               ...
               files={name: sha(folder/name) for name in ('stdout.txt', 'stderr.txt', 'timings.bin.gz')})
...
def replay_check(binary, folder):
    ...
    def states(path):
        return [json.loads(s.split(' ', 1)[1]) for s in path.read_text().splitlines() if s.startswith('FT1536_BATCH ')]
    a, b = states(folder/'stdout.txt'), states(folder/'replay.stdout')
    if code or not a or a != b:
        raise RuntimeError('Raw/statistics replay mismatch: ' + str(folder))
    return dict(status='PASS', batches=len(a), raw_sha256=sha(folder/'timings.bin.gz'),
