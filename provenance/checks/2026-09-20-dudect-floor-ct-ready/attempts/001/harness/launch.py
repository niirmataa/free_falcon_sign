...
def launch(work):
    ...
    fs = os.statvfs(work)
    required = 28800 * (2 * 2**20) + 17 * 2**30
    if fs.f_bavail * fs.f_frsize < required:
        raise RuntimeError('Insufficient free disk for eight-hour full logs plus the reserve')
    unit = profile['unit']
    ...
    argv = ['systemd-run','--user','--service-type=exec','--unit='+unit,
            '--description=FT1536 '+profile_name+' dudect, bounded eight-hour campaign',
    ...
    info = dict(request_utc=utc(),unit=unit,argv=argv,exit_code=p.returncode,
                ...
                source_manifest_sha256=profile['sha256'])
