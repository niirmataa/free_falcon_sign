...
# FIX_001 (independent review I2): mid/rad are BINARY floats; their exact
# values are taken with .exact_rational() -- QQ(RealNumber) is NOT a
# guaranteed exact binary conversion and must not silently widen the bound.
# The true value lies in [mid-rad, mid+rad] (rad is an upper bound of the
# error); we widen to +/- 2*rad for slack.  REAL ENCLOSURE SIZE: the Arb256
# ball radius propagated to the tail is ~1.06e-82 (absolute) -- the earlier
# ~1e-305 claim belonged to a superseded Decimal draft and is withdrawn.
mid = e_ball.mid()                   # RealNumber (binary midpoint)
rad = e_ball.rad()                   # RealNumber (binary |error| upper bound)
mid_q = mid.exact_rational()         # exact binary value as QQ
rad_q = rad.exact_rational()         # exact binary value as QQ
E_lo = mid_q - 2*rad_q               # <= exp(-x)
E_HI = mid_q + 2*rad_q               # >= exp(-x)
...
half_width_q = (T_hi - T_lo) / 2          # = 2 * rad_q * S (widening slack)
ball_radius_q = rad_q * S                 # arb radius carried to the tail


def sci_dec(f, sig=10):
    """Display-only scientific string of a positive QQ (not certified)."""
    return ("%." + str(sig - 1) + "e") % float(f)


ball_radius_disp = sci_dec(ball_radius_q)
half_width_disp = sci_dec(half_width_q)
...
rep = {
    ...
    "method": {
        ...
        "exp_enclosure": "RealBallField(256) exp (arb: rigorous outward "
                         ...
                         "with mid/rad converted by .exact_rational() "
                         "(exact binary values; QQ(RealNumber) is NOT exact)",
    ...
    "tail_interval": {
        "lo_decimal_100": qq_floor_dec(T_lo, 100),
        "hi_decimal_100": qq_ceil_dec(T_hi, 100),
        "center_display_50": center_disp,
        "arb256_ball_radius_abs_display": ball_radius_disp,
        "certified_interval_half_width_display": half_width_disp,
        "precision_note": "REAL enclosure of this run: Arb256 ball radius "
                          "carried to the tail ~ 1.06e-82 (absolute); the "
                          "certified interval half-width is 2*radius (the "
                          "+/- 2*rad widening, exact_rational conversion). "
                          "Stored endpoints are outward-rounded decimals at "
                          "100 fractional digits. The ~1e-305 figure of "
                          "RUN_003 texts/PDF belonged to a superseded "
                          "Decimal draft and is WITHDRAWN for this run.",
    },
...
print("arb256 ball radius (abs):", ball_radius_disp,
      "| certified half-width:", half_width_disp)
