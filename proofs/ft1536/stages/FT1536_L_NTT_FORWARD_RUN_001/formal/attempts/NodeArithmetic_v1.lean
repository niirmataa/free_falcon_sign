...
theorem powFast_correct (z : Int) (n : Nat) : powFast z n=z^n%18433 := by
  ...
  | ind n ih =>
    ...
    · rename_i hn
      ...
      rcases hm with hm|hm <;> simp [hm,Int.mul_emod]
...
theorem nodeCheck_sound (i : Nat) (w : Int) (j : Fin 3) (h : nodeCheck i w j=true) :
  ...
  have hs : ((decide (powFast (nodeFromWord w j) 768=rootLabel (3*(i-512))%18433) &&
      ...
      ([0,1,2] : List (Fin 3)).all (fun k => decide ((cubeFExpr w fW j).eval (basis k)=powFast (nodeFromWord w j) k.val))=true := Bool.and_eq_true.mp h
  obtain ⟨ha,hc⟩:=Bool.and_eq_true.mp hs.1
  obtain ⟨hr,hb⟩:=Bool.and_eq_true.mp ha
  refine ⟨?_,?_,?_,?_⟩
