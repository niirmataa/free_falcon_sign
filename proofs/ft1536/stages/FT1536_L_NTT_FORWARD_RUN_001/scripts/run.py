"""Bounded jobs; writable mount, cache and all compiler output restricted to W."""
...
env=dict(os.environ);env.update(HOME=str(W/'cache/home'),TMPDIR=str(W/'tmp'),TMP=str(W/'tmp'),TEMP=str(W/'tmp'),
 ...
 PYTHONDONTWRITEBYTECODE='1',PYTHONPYCACHEPREFIX=str(W/'cache/pycache'),MAMBA_ROOT_PREFIX='/home/footfalcon/miniforge3',
 GIT_OPTIONAL_LOCKS='0',OMP_NUM_THREADS='1',OPENBLAS_NUM_THREADS='1',LEAN_PATH=str(W/'formal'))
box=['/usr/bin/bwrap','--die-with-parent','--unshare-net','--unshare-pid','--ro-bind','/','/','--bind',str(W),str(W),
 '--proc','/proc','--dev','/dev','--chdir',str(W)]
if (W/'source').exists():box+=['--ro-bind',str(W/'source'),str(W/'source')]
box+=['--']
def bounds():
    resource.setrlimit(resource.RLIMIT_CPU,(limit,limit+1));resource.setrlimit(resource.RLIMIT_CORE,(0,0))
    resource.setrlimit(resource.RLIMIT_AS,(8*1024**3,8*1024**3));resource.setrlimit(resource.RLIMIT_NOFILE,(256,256))
...
with (W/'executor.lock').open('a') as lock:
    ...
    row=dict(start_utc=start,cwd=str(W),argv=argv,sandbox=box,exit_code=p.returncode,timeout=timed,
        wall_limit_seconds=limit,cpu_limit_seconds=limit,address_space_bytes=8*1024**3,elapsed_seconds=time.monotonic()-tick,
        **paths,stream_sha256={'stdout':hashlib.sha256(out).hexdigest(),'stderr':hashlib.sha256(err).hexdigest()},
