...
cmds=[['/usr/bin/gcc','--version'],['/usr/bin/gcc','-dumpmachine'],
...
rows=[];text=[]
for c in cmds:
 p=subprocess.run(c,capture_output=True,timeout=60);assert p.returncode==0
 rows.append(dict(argv=c,exit_code=p.returncode,stdout=p.stdout.decode(),stderr=p.stderr.decode()));text+=['$ '+' '.join(c),p.stdout.decode(),p.stderr.decode()]
