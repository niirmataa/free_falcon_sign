...
for r in rows:
    ...
    assert abs(df-I(dq(r['delta_falcon']))).upper().exact_rational()<QQ(1)/10^38
    assert abs(de-I(dq(r['delta_estimator']))).upper().exact_rational()<QQ(1)/10^38
    ...
    enum_larger=enum_bits.lower().exact_rational()>lll_bits.upper().exact_rational()
    base=enum_bits if enum_larger else lll_bits
    other=lll_bits if enum_larger else enum_bits
    total=base+(1+((other-base)*ln2).exp()).log()/ln2
    diff=abs(total-I(dq(r['lambda_enum_CN12']))).upper().exact_rational()
    mant,exponent=r['lambda_enum_CN12'].lower().split('e')
    display_unit=QQ(10)^(ZZ(exponent)-len(mant.partition('.')[2]))
    cn_ok=bool(diff<display_unit)
    sr=next(x for x in surface if x['scheme']==r['scheme'] and x['attack']==r['attack'] and x['variant']==r['variant'] and x['cost_model']=='CheNgu12')
    assert abs(total-I(str(sr['log2_rop_+0']))).upper().exact_rational()<QQ(1)/10^8
    checks.append({'scheme':r['scheme'],'attack':r['attack'],'variant':r['variant'],
                   'delta_falcon_agrees':True,'delta_estimator_agrees':True,
                   'CN12_agrees_within_one_display_unit':cn_ok,
                   'CN12_diff_display':str(I(diff)),'CN12_display_unit':str(display_unit)})
...
assert all(r['CN12_agrees_within_one_display_unit'] for r in checks),checks
