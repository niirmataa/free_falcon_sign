"""External pin/full manifest before fresh DEST; only fresh bounded single-worker jobs."""
...
entries=verify_manifest(W,manifest,args.external_sha256);expected=json.loads(member(W,'SEMANTIC_FILES.json').read_text())['matches']
...
r=dict(status='RUNNING',mode='PREFREEZE_REHEARSAL' if args.rehearsal else 'STANDARD',input_manifest=manifest,input_manifest_sha256=args.external_sha256,
 source=str(W),destination=str(D),seed_members=len(seed),cached_olean_or_binaries_used=False,source_changed=False,jobs=[])
tick=time.monotonic()
def save():(D/'REPLAY_RESULT.json').write_text(json.dumps(r,indent=2)+'\n')
def job(tag,seconds,script,*arguments,asan=False,sage=False):
 interpreter=['/home/footfalcon/.local/bin/sage'] if sage else ['/usr/bin/python3','-B']
 cmd=['/usr/bin/python3','-B','scripts/run.py',str(seconds)]+interpreter+['scripts/'+script+'.py',*arguments]
 ...
 row=dict(tag=tag,argv=cmd,cwd=str(D),exit_code=p.returncode,timeout=timed,elapsed_seconds=time.monotonic()-start,stdout=so.relative_to(D).as_posix(),stderr=se.relative_to(D).as_posix(),stdout_sha256=sha(so),stderr_sha256=sha(se))
 r['jobs'].append(row);save();print(json.dumps(dict(job=tag,exit_code=p.returncode,elapsed_seconds=row['elapsed_seconds'])),flush=True)
 assert not timed and p.returncode==0,tag
try:
 save();job('toolchain',60,'toolchain');job('transport',120,'source_transport');job('source_checks',60,'source_checks')
 job('tower_numeric',120,'tower_certificate',sage=True);job('kernel',240,'kernel_build');job('audit',90,'audit')
 job('fixtures',240,'fixtures');job('normal',120,'controls','normal');job('sanitizers',120,'controls','san',asan=True)
 job('mutations',240,'mutations');job('composition',60,'composition_certificate');job('certificate',60,'certificate')
 assert set(semantic_paths(D))=={x['path'] for x in expected},'changed generated scope'
 ...
 r.update(status='FRESH_REPLAY_PASS',matches=expected,semantic_matches=len(expected),elapsed_seconds=time.monotonic()-tick);save()
 print(json.dumps({k:v for k,v in r.items() if k not in ['matches','jobs']},indent=2))
