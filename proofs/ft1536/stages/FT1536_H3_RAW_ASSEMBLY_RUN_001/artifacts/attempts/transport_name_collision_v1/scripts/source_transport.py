...
for index,unit in enumerate(units):
 ...
 for name,definition in extract(p.stdout.decode()).items():
  if name in functions:assert re.sub(r'\s+','',functions[name])==re.sub(r'\s+','',definition),name
  functions[name]=definition;owners.setdefault(name,[]).append(unit)
def calls(body):return sorted(set(re.findall(r'\b([A-Za-z_]\w*)\s*\(',body.split('{',1)[1]))&functions.keys())
graph={n:calls(d) for n,d in functions.items()}
def closure(seeds):
...
out=dict(status='PASS_SOURCE_TRANSPORT_AND_RAW_CUT',baseline_manifest_sha256=sha(I/'PREVIOUS_SOURCE.sha256'),candidate_manifest_sha256=sha(I/'CANDIDATE.sha256'),
 ...
 tables_sha256=tables,consumed_body_hashes={n:hashlib.sha256(functions[n].encode()).hexdigest() for n in sorted(set(raw+emitted))},
