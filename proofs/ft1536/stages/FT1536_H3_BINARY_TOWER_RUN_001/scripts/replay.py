...
def main():
 ...
 for rel in json.loads(member(W,'artifacts/formal_audit.json').read_text())['sources_sha256']:
  if Path(rel).name not in ['TowerAudit.lean','TowerTypes.lean']:cp(rel)
 for rel in ['INDUCTION.md','LEVEL7.md','ASSEMBLY_INTERFACE.md','CLAIM.md','NEXT_INTERFACE.md','SOURCE_MODEL_BINDING.md','REUSED_RESULTS.md','checks/tower.c','artifacts/inherited_order.json']:cp(rel)
 for name in ['run','lean','dyadic','fp_literal','replaylib','backend','root_model','node_model','node2_model','toolchain','half_model','tower_model','fixtures','certificate','controls','check_lean_values','oracle','source_binding','audit','ledger']:cp('scripts/'+name+'.py')
 ...
 out=dict(status='FRESH_REPLAY_PASS',matches=matches,mode='rehearsal' if rehearsal else 'standard',expected_outputs_sha256=pin,
  proof_kind='MIXED_KERNEL_AND_UNIVERSAL_ANALYTICAL_SOURCE_PROOF',full_binary_tower_theorem_kernelized=False,
  levels_proved=[7,6,5,4,3,2,1],no_carried_olean_binary_cache=True,originals_required=False,asan_ubsan_replayed=True,
  binary_tower_proved=True,remaining_binary_subtrees_proved=True,source_inner7_totality_proved=True,
