...
W=Path.cwd();I=W/'inputs/bootstrap';TASK=Path('/home/footfalcon/Dokumenty/FT1536_ZADANIE_ASTRA_H3_BINARY_TOWER_2026-09-19.md')
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
assert W==Path('/home/footfalcon/free_falcon_sign/proofs/ft1536/work/FT1536_H3_BINARY_TOWER_RUN_001')
assert sha(TASK)=='5d00e65d8a7f59d9b46e6d133eb81de1da374d6764b4600183e018710931cc14'
assert sha(I/'MANIFEST.sha256')=='9bd79a0179910af92457ce6db5419aa859017499a79c575fb8256b5bfc92d7a1'
...
assert len(rows)==137 and {p.relative_to(I).as_posix() for p in I.rglob('*') if p.is_file()}==set(rows)|{'MANIFEST.sha256'}
orig=json.loads((I/'ORIGINS.json').read_text());assert len(orig['files'])==135 and orig['base_commit']=='b27a0557f785eabd30632eeea9da6b9cf170a7d0'
...
reuse=[]
for name in ['lean','dyadic','fp_literal','replaylib','backend','root_model','node_model','node2_model','half_model','fixtures','toolchain','check_lean_values']:
 src=I/'NODE2/scripts'/(name+'.py');dst=W/'scripts'/(name+'.py');shutil.copyfile(src,dst)
...
def visit(mod):
 ...
 src=I/'NODE2/formal'/(mod+'.lean');assert sha(src)==rows['NODE2/formal/'+mod+'.lean']
 for dep in re.findall(r'^import (\w+)',src.read_text(),re.M):
  if (I/'NODE2/formal'/(dep+'.lean')).exists():visit(dep)
 dst=W/'formal'/(mod+'.lean');shutil.copyfile(src,dst);order.append(mod)
 reuse.append(dict(input=str(src.relative_to(W)),copy=str(dst.relative_to(W)),sha256=sha(dst),byte_identical=True))
visit('BinaryConstants');visit('HalfMain')
...
diff=''.join(difflib.unified_diff((I/'NODE2/scripts/run.py').read_text().splitlines(True),(W/'scripts/run.py').read_text().splitlines(True),fromfile='bootstrap/NODE2/scripts/run.py',tofile='scripts/run.py'))
...
out=dict(members=len(rows),originals=135,source_files=17,public_inputs=len(provenance),all_pins_match=True,exact_scope=True,inherited_order=order)
