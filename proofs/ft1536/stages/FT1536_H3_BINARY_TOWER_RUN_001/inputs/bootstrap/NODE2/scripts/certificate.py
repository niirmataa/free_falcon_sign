...
fields=['s0_real_lower','d11_real_lower','s0_real_upper','d11_real_upper','s0_imag_abs_upper','d11_imag_abs_upper','L_norm_upper','L_error_norm','d11_error_norm']
text=(W/'formal/BinaryConstants.lean').read_text()
for r in records:
 line=re.search(r'def c'+str(r['branch'])+str(r['diagonal'])+r' : Constants := (.*)',text).group(1)
 vals=[QQ(int(n))/int(d) for n,d in re.findall(r'\((-?\d+),(\d+)\)',line)]
 assert vals==[QQ(r[k]) for k in fields]
out=dict(schema='H3_NODE2_NUMERIC_CERTIFICATE_V1',status='PASS_UNIFORM_FIRST_BINARY_COMPOSITION',level=dict(split_logn=9,ldl_logn=8,full=0,positions='2*3*128'),constants=records,kernel_constant_fields_match=fields,
