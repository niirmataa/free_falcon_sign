...
def main():
 ...
 for rel in json.loads(member(W,'artifacts/formal_audit.json').read_text())['sources_sha256']:
  if Path(rel).name not in ['NodeAudit.lean','NodeTypes.lean']:cp(rel)
 for rel in ['ANALYTIC_PROOF.md','ROOT_CONSUMPTION.md','NODE3_FRAME.md','CLAIM.md','NEXT_INTERFACE.md','SOURCE_MODEL_BINDING.md','REUSED_RESULTS.md',
  'checks/node.c','artifacts/inherited_order.json']:cp(rel)
 for name in ['run','lean','dyadic','fp_literal','replaylib','backend','root_model','toolchain','node_model','fixtures','certificate','controls','check_lean_values','oracle','source_binding','audit','ledger']:cp('scripts/'+name+'.py')
 ...
 out=dict(status='FRESH_REPLAY_PASS',matches=matches,mode='rehearsal' if rehearsal else 'standard',expected_outputs_sha256=pin,
  proof_kind='MIXED_KERNEL_AND_UNIVERSAL_ANALYTICAL_SOURCE_PROOF',full_node3_theorem_kernelized=False,
