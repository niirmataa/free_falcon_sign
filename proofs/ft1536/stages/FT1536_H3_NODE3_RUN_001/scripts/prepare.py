...
W=Path.cwd();I=W/'inputs/bootstrap';TASK=Path('/home/footfalcon/Dokumenty/FT1536_ZADANIE_ASTRA_H3_NODE3_2026-09-19.md')
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
assert W==Path('/home/footfalcon/free_falcon_sign/proofs/ft1536/work/FT1536_H3_NODE3_RUN_001')
assert sha(TASK)=='9960e9a2a5f749761f8c7aba4d861e8b4fd5ba98a6523cb0d4d2cae61bbb1d30'
assert sha(I/'MANIFEST.sha256')=='426db8a67b74de0de0141a8d9ca406fca7a2bdad45a42e97a913bad4d62b6ca3'
...
assert len(rows)==108 and {p.relative_to(I).as_posix() for p in I.rglob('*') if p.is_file()}==set(rows)|{'MANIFEST.sha256'}
orig=json.loads((I/'ORIGINS.json').read_text());assert len(orig['files'])==106 and orig['base_commit']=='3d6bf58b833d729983718f80da020118fa94188a'
...
reuse=[]
for name in ['lean','dyadic','fp_literal','replaylib','backend','root_model','toolchain']:
 src=I/'ROOT/scripts'/(name+'.py');dst=W/'scripts'/(name+'.py');shutil.copyfile(src,dst)
...
def visit(mod):
 ...
 src=I/'ROOT/formal'/(mod+'.lean');assert sha(src)==rows['ROOT/formal/'+mod+'.lean']
 for dep in re.findall(r'^import (\w+)',src.read_text(),re.M):
  if (I/'ROOT/formal'/(dep+'.lean')).exists():visit(dep)
...
diff=''.join(difflib.unified_diff((I/'ROOT/scripts/run.py').read_text().splitlines(True),(W/'scripts/run.py').read_text().splitlines(True),fromfile='bootstrap/ROOT/scripts/run.py',tofile='scripts/run.py'))
...
out=dict(members=len(rows),originals=106,source_files=17,public_inputs=len(provenance),all_pins_match=True,exact_scope=True,inherited_order=order)
