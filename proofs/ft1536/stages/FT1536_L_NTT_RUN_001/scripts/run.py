#!/usr/bin/env python3
"""Bounded command receipt and actual write isolation for this new W."""
...
with (W/'executor.lock').open('a') as lock:
    ...
    row=dict(start_utc=start,cwd=str(W),argv=argv,sandbox=box,exit_code=p.returncode,timeout=timed,
      wall_limit_seconds=limit,cpu_limit_seconds=limit,elapsed_seconds=time.monotonic()-tick,**streams,
