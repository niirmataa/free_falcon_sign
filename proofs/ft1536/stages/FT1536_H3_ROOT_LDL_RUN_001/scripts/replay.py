...
def main():
 ...
 for rel in json.loads(member(W,'artifacts/formal_audit.json').read_text())['sources_sha256']:
  if Path(rel).name not in ['RootAudit.lean','RootTypes.lean']:cp(rel)
 for rel in ['ANALYTIC_PROOF.md','EMITTED_BINDING.md','ROOT_FRAME.md','CLAIM.md','NEXT_INTERFACE.md','SOURCE_MODEL_BINDING.md','REUSED_RESULTS.md',
  'checks/root.c','checks/keygen_map.c','artifacts/inherited_order.json']:cp(rel)
 for name in ['run','lean','dyadic','fp_literal','replaylib','backend','root_model','controls','certificate','emitted_certificate','oracle','check_lean_values','source_binding','ledger','audit','toolchain']:cp('scripts/'+name+'.py')
 ...
 jobs=[('kernel',180,False,['python3','-B','scripts/audit.py','--build']),('normal',240,False,['python3','-B','scripts/controls.py','normal']),
  ...
  ('numeric',120,False,['/home/footfalcon/.local/bin/sage','scripts/certificate.py']),('emitted',120,False,['/home/footfalcon/.local/bin/sage','scripts/emitted_certificate.py']),
  ('oracle',120,False,['/home/footfalcon/.local/bin/sage','scripts/oracle.py']),('binding',30,False,['python3','-B','scripts/source_binding.py']),
  ('ledger',30,False,['python3','-B','scripts/ledger.py']),('toolchain',90,False,['python3','-B','scripts/toolchain.py'])]
 records=[]
 ...
 out=dict(status='FRESH_REPLAY_PASS',matches=matches,mode='rehearsal' if rehearsal else 'standard',expected_outputs_sha256=pin,
  local_proof_kind='MIXED_KERNEL_AND_UNIVERSAL_ANALYTICAL_SOURCE_PROOF',full_root_theorem_kernelized=False,
  no_carried_olean_binary_cache=True,originals_required=False,asan_ubsan_replayed=True,H3_range_proved=False,global_reachability_proved=False,full_internal_tree_proved=False,sampler_law_proved=False)
