...
(W/'artifacts/replay_evidence.json').write_text(json.dumps(dict(copied=len(rows),files=rows,semantic_matches=len(receipt['matches']),
 note='Verbatim full streams/receipts. Analytical proof text remains a separately reviewed proof layer.'),indent=2)+'\n')
