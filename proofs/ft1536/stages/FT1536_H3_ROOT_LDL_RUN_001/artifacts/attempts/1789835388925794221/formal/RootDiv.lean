...
theorem step_range (v r q : Nat) (hr : r<2*v) : (step v (r,q)).1<2*v := by
  unfold step
  split <;> simp only [Prod.fst] <;> omega
theorem step_invariant (v r q : Nat) :
    (step v (r,q)).1+(step v (r,q)).2*v=2*(r+q*v) := by
  unfold step
  split
  · rename_i h
    simp only [Prod.fst,Prod.snd,Nat.mul_add,Nat.add_mul,Nat.mul_assoc,Nat.one_mul]
    omega
  · simp only [Prod.fst,Prod.snd,Nat.mul_add,Nat.mul_assoc]
