...
theorem reduced_low_count (k : Nat) (hk : k<768) : reducedCount k=2303-k := by
  simp only [reducedCount,if_pos hk,convolutionCount]
  split <;> split <;> split <;> omega
theorem reduced_high_count (k : Nat) (hk : 768≤k ∧ k<1536) : reducedCount k=2304 := by
  simp only [reducedCount,if_neg (show ¬k<768 by omega),convolutionCount]
  split <;> split <;> omega
...
theorem gram_pair_bounds (a b : Int) :
  ...
  have hs:=Int.mul_self_nonneg (a+b)
  have hd:=Int.mul_self_nonneg (a-b)
