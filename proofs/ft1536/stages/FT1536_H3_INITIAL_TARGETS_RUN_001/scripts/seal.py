...
prefix=dict(path=p.relative_to(W).as_posix(),sha256=sha(p),bytes=len(data),records=len(data.splitlines()),scope='Completed exact prefix before seal; finalization/post-freeze receipts only under new tmp destinations')
