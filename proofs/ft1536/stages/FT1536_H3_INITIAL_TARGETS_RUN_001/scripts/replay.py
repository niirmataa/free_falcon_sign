...
try:
 ...
 job('numeric',180,'numeric_certificate',sage=True);job('kernel',240,'kernel_build');job('audit',90,'audit');job('fixtures',180,'fixtures')
 job('normal',120,'controls','normal');job('sanitizers',120,'controls','san',asan=True);job('oracle',240,'oracle',sage=True);job('mutations',180,'mutations');job('ledger',60,'ledger');job('certificate',60,'certificate')
