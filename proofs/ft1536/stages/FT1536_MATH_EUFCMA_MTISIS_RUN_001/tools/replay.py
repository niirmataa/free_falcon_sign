#!/usr/bin/env python3
"""Read-only sealed bundle -> NEW destination. Organization/hashes, no math in Python."""
import argparse
...
def verify_manifest(bundle,manifest,pin):
    ...
    for line in manifest.read_text().splitlines():
        ...
        assert p.resolve().is_relative_to(bundle),(name,'escape')
...
def run_step(dest,name,argv,cwd,env):
    ...
    sandbox=['/usr/bin/bwrap','--die-with-parent','--unshare-net','--ro-bind','/','/',
             ...
             '--bind',str(dest/'tmp'),'/tmp','--chdir',str(cwd),
    ...
    rec=dict(name=name,argv=argv,sandbox_argv=sandbox,cwd=str(cwd),start=start,stop=now(),
        elapsed_s=round(time.monotonic()-t0,3),exit_code=code,wall_limit_s=1800,
        network='bwrap --unshare-net',address_space_bytes=12884901888,
        lean_heap_MiB=6144,cumulative_child_maxrss_kib=resource.getrusage(resource.RUSAGE_CHILDREN).ru_maxrss,
...
def main():
    ...
    ap.add_argument('--library-map',help='optional relocation JSON: library -> {source,build}; all bytes still pinned')
    ...
    required=['BUILD.json','FORMAL_EXPORTS.json','LIBRARY_CLOSURE.json','TOOLCHAIN.json',
              'EXPECTED.json','tools/replay.py','sage/check_bounds.sage']
    ...
    assert 'proofs/ft1536/work' in str(dest) or 'proofs/ft1536/replay-work' in str(dest),'DEST_MUST_BE_DURABLE_PROJECT_WORK'
    ...
    for item in closure['modules']:
        ...
        assert sha(Path(roots_item['source'])/relative)==item['source_sha256'],('LIBRARY_SOURCE_CHANGED',item['module'])
        assert sha(bundle/item['source'])==item['source_sha256']
        for art in item['artifacts']:
            assert sha(Path(roots_item['build'])/art['path'])==art['sha256'],('CACHE_PIN_MISMATCH',item['module'],art['path'])
            artifacts+=1
    print(f'MANIFEST_PASS {len(checked)} members; LIBRARY_PASS {len(closure["modules"])} modules/{artifacts} artifacts',flush=True)
    if args.verify_only: return
    dest.mkdir(parents=True,exist_ok=False)
    for sub in ['logs','tmp','home','cache','lib','work']:(dest/sub).mkdir()
    shutil.copytree(bundle/'formal',dest/'formal')
    shutil.copytree(bundle/'sage',dest/'sage')
    env=os.environ.copy()
    env.update(HOME=str(dest/'home'),TMPDIR=str(dest/'tmp'),TMP=str(dest/'tmp'),TEMP=str(dest/'tmp'),
        DOT_SAGE=str(dest/'home/.sage'),XDG_CACHE_HOME=str(dest/'cache'),MPLCONFIGDIR=str(dest/'cache/mpl'),
        PYTHONDONTWRITEBYTECODE='1',OPENBLAS_NUM_THREADS='1',OMP_NUM_THREADS='1',
        LEAN_PATH=':'.join([str(dest/'lib')]+[v['build'] for v in roots.values() if v!=roots['lean']]))
    ...
    try:
        ...
        receipts.append(run_step(dest,'sage_controls',[sage,str(dest/'sage/check_bounds.sage')],dest/'work',env))
        generated=dest/'work/generated/Certificate.lean'
        assert sha(generated)==sha(bundle/'formal/FT1536/Certificate.lean'),'SAGE_LEAN_CERTIFICATE_MISMATCH'
        shutil.copyfile(generated,dest/'formal/FT1536/Certificate.lean')
        ...
        for mod in build['modules']:
            ...
            rec=run_step(dest,mod.replace('.','_'),[lean]+build['lean_flags']+['-o',str(obj),str(source)],dest/'formal',env)
        ...
        axioms={}
        for m in re.finditer(r"'([^']+)' (?:depends on axioms: \[([^\]]*)\]|does not depend on any axioms)",audit):
            axioms[m[1]]=[re.sub(r'\.\{[^}]+\}', '', s.strip())
                         for s in (m[2] or '').split(',') if s.strip()]
        names=[e['name'] for e in json.loads((bundle/'FORMAL_EXPORTS.json').read_text())['exports']]
        ...
        (dest/'AXIOMS.json').write_text(json.dumps(axioms,indent=2,sort_keys=True)+'\n')
        status='PASS'
        summary=dict(status=status,manifest=args.manifest,external_manifest_sha256=args.manifest_sha,
            checked_members=len(checked),library_modules=len(closure['modules']),library_artifacts=artifacts,
            exports_audited=len(names),comparisons=comparisons,own_cache_fresh=True,
            inputs_and_outputs_in_new_dest=True,network='bwrap --unshare-net',receipts=receipts)
        (dest/'REPLAY_RESULT.json').write_text(json.dumps(summary,indent=2)+'\n')
    finally:
        (dest/'EXECUTION_RECEIPTS.json').write_text(json.dumps(dict(status=status,jobs=receipts),indent=2)+'\n')
        (dest/'SAGE_RUNS.json').write_text(json.dumps([r for r in receipts if r['name'].startswith('sage')],indent=2)+'\n')
        (dest/'COMMANDS.log').write_text(''.join(json.dumps(r)+'\n' for r in receipts))
    # All sealed inputs must still have their pinned bytes after execution.
