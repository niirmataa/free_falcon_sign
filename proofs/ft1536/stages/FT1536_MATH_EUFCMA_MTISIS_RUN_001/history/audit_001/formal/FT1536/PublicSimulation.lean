...
def encodeCoord (x : ℤ) (hx : -65535 ≤ x ∧ x ≤ 65535) : Fin 131071 :=
  ⟨(x+65535).toNat, by omega⟩

theorem decode_encodeCoord (x : ℤ) (hx : -65535 ≤ x ∧ x ≤ 65535) :
    ((encodeCoord x hx).val : ℤ) - 65535 = x := by
  simp only [encodeCoord]
  omega

def encodeVec (v : Vec) (hv : ∀ i,
    (-65535 ≤ (v i).1 ∧ (v i).1 ≤ 65535) ∧
    (-65535 ≤ (v i).2 ∧ (v i).2 ≤ 65535)) : BoxVec :=
  fun i => (encodeCoord (v i).1 (hv i).1, encodeCoord (v i).2 (hv i).2)

theorem decode_encodeVec (v : Vec) (hv : ∀ i,
    (-65535 ≤ (v i).1 ∧ (v i).1 ≤ 65535) ∧
    (-65535 ≤ (v i).2 ∧ (v i).2 ≤ 65535)) : decodeVec (encodeVec v hv) = v := by
  funext i
  exact Prod.ext (decode_encodeCoord (v i).1 (hv i).1)
    (decode_encodeCoord (v i).2 (hv i).2)

theorem full_norm_support_in_box (v w : Vec) (h : Q (v,w) < B) :
    ∃ z : BoxPair, decode z = (v,w) := by
  have hv := fun i => all_coord_bounds v w h i
  refine ⟨(encodeVec v (fun i => ⟨(hv i).1, (hv i).2.1⟩),
    encodeVec w (fun i => ⟨(hv i).2.2.1, (hv i).2.2.2⟩)), ?_⟩
  simp [decode, decode_encodeVec]
