...
theorem chi2_toReal (j p : Law α) (h : AC j p) :
    (chi2 j p).toReal = second j p - 1 := by
  have hn : 0 ≤ second j p - 1 := by
    have hh := energy_ge_one j p (densityOfAC j p h)
    rw [energy_eq_second] at hh
    linarith
  simp [chi2, h, ENNReal.toReal_ofReal hn]

theorem chi2_finite (j p : Law α) (h : AC j p) : chi2 j p ≠ ⊤ := by
  simp [chi2, h]
...
theorem joint_chi2_finite (j p : Law α) (l k : α → Law β) (h : AC j p)
    (hk : ∀ x, j.mass x ≠ 0 → AC (l x) (k x)) :
    1 + (chi2 (joint j l) (joint p k)).toReal =
      ∑ x, (j.mass x^2 / p.mass x) * (1 + (chi2 (l x) (k x)).toReal) := by
  rw [chi2_toReal _ _ (joint_ac j p l k h hk)]
  calc
    _ = second (joint j l) (joint p k) := by ring
    _ = _ := joint_chi2 j p l k
    _ = _ := by
      apply sum_congr rfl
      intro x _
      by_cases hx : j.mass x = 0
      · simp [hx]
      · rw [chi2_toReal _ _ (hk x hx)]; ring
