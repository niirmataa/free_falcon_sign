...
theorem reduce_center (c : Rq) : reduceVec (centerRq c) = c := by
  have hc (x : ZMod 18433) : ((center x.val : ℤ) : ZMod 18433) = x := by
    unfold center
    rw [Int.cast_sub, ZMod.intCast_mod ((x.val : ℤ)+9216) 18433]
    simp
  funext i
  exact Prod.ext (hc (c i).1) (hc (c i).2)
...
noncomputable def poly (a : Rq) : Polynomial (ZMod 18433) :=
  ∑ i, (Polynomial.C (a i).1 * X^i.val + Polynomial.C (a i).2 * X^(i.val+768))
noncomputable def modulus : Polynomial (ZMod 18433) := X^1536-X^768+1
