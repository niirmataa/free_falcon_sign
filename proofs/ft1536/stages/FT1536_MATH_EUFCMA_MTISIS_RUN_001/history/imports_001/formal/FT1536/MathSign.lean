...
theorem cap_some (trial : Law (Option α)) (n : ℕ) (x : α) :
  ...
  | succ n ih =>
    ...
    simp only [Option.some.injEq, if_false, Option.some_ne_none, mul_ite, mul_one,
      mul_zero, sum_ite_eq', mem_univ, if_true]
    rw [ih]
    have geo : trial.mass none * (∑ i ∈ range n, trial.mass none ^ i) + 1 =
        ∑ i ∈ range (n+1), trial.mass none ^ i := by
      rw [sum_range_succ']
      simp only [pow_zero, ← mul_pow, pow_succ', ← mul_sum]
    nlinarith [geo]
...
def emit [DecidableEq β] (encode : α → Option β) : Option α → Option β
  | none => none
  | some x => encode x

noncomputable def observe [Fintype β] [DecidableEq β] [Fintype R] [DecidableEq R]
    (prefix : Bool) (nonce : Law R) (body : R → Law (Option α))
    (encode : α → Option β) : Law (Observation R β) :=
  if prefix then nonce.bind (fun r => (body r).map fun z => some (r, emit encode z))
  else Law.pure none
