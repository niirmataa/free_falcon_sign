"""Bounded W-only jobs; dedicated explicit timing mode admits at most1800s."""
...
timing=os.environ.get('FLOOR_CT_TIMING')=='1';assert argv and 1<=limit<=(1800 if timing else 240)
...
for rel in ['inputs/bootstrap','baseline','vendor','harness']:
 if (W/rel).exists():box+=['--ro-bind',str(W/rel),str(W/rel)]
if (W/'artifacts/candidate_freeze.json').exists():box+=['--ro-bind',str(W/'candidate/source'),str(W/'candidate/source')]
box+=['--']
def bounds():
...
with (W/'executor.lock').open('a') as lock:
 ...
 row=dict(start_utc=start,cwd=str(W),argv=argv,sandbox=box,exit_code=p.returncode,timeout=timed,wall_limit_seconds=limit,cpu_limit_seconds=limit,
  address_space_bytes=None if asan else 8*1024**3,asan_shadow_reservation=asan,timing_mode=timing,elapsed_seconds=time.monotonic()-t,**paths,
