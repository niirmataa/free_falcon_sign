...
names = [([], ), ([1,2], ), (list(range(5)), )]
framed = [(ZZ(3), [9,9,9]), (ZZ(4), []), (ZZ(65535), [1])]
seen = set()
for (nm,) in [(list(n),) for (n,) in names]:
    pass
rendered = [list(n) for (n,) in names] + [sign_name(r, m) for (r, m) in framed]
for a in rendered:
    for b in rendered:
        if a != b:
...
assert st_real.used == ZZ(3)      # A(fresh), B(sign: none), C(final fresh) -> 2 hash + 1 final
...
assert st_real.entries[tuple(signC)][1] == targets[2]
...
ops_conf = [('s', [ZZ(0)], signB), ('h', signB), ('f', signC, ZZ(2))]
...
jS = [QQ(5/8), QQ(3/8)]                       # simulator Sign kernel
e_sign = second(jS, pS) - 1
assert e_sign == QQ(1)/32
...
assert phi(QQ(0), QQ(1/3)) == R(QQ(1)/3)
...
def mut_target_reindex():
    # mutated: final entry bound to a different index value
    bound = st_real.entries[tuple(signC)][1] == targets[0]
    return bound                          # extraction index/value check breaks
...
theorem paid_counter_exact :
  ((1 + (1/32:ℝ)) ^ 2) = 1089/1024 ∧
  ((1 + (1/32:ℝ)) ^ 4) = 1185921/1048576 ∧
  (1 + (1/32:ℝ)) ^ 2 < (1 + (1/32:ℝ)) ^ 4 := by norm_num
