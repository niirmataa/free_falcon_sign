"""Read-only integrity and pin validation; not a new mathematical status."""
import json,sys
...
assert result['status']=='H3_NODE2_PROVED_FOR_PINNED_MODEL' and result['full_node2_theorem_kernelized'] is False
assert result['level']['split_logn']==9 and result['level']['ldl_logn']==8 and result['level']['full']==0
...
matches={r['path']:r['sha256'] for r in receipt['matches']};assert len(matches)==len(receipt['matches'])==160
...
boot=W/'inputs/bootstrap';entries=verify_manifest(boot,'MANIFEST.sha256',result['pins']['bootstrap_manifest_sha256']);assert len(entries)==120
assert {p.relative_to(boot).as_posix() for p in boot.rglob('*') if p.is_file()}==set(entries)|{'MANIFEST.sha256'}
orig=json.loads((boot/'ORIGINS.json').read_text());assert len(orig['files'])==118
...
assert len(provenance)==124 and inputs=={r['path']:r['sha256'] for r in provenance}
for r in provenance:assert scope[r['copy']]==r['sha256']
ledger=json.loads(member(W,'BOUND_LEDGER.json').read_text());node=json.loads(member(W,'NODE2_CERTIFICATE.json').read_text())
assert ledger['status']==node['status']==result['status'] and ledger['constants']==node['constants']==result['uniform_c2']
for evidence in node['evidence']+[e for row in ledger['rows'] for e in row['evidence']]:assert scope[evidence['path']]==evidence['sha256']
audit=json.loads(member(W,'artifacts/formal_audit.json').read_text());assert audit['all_final_logs_clean'] and not audit['full_node2_theorem_kernelized']
