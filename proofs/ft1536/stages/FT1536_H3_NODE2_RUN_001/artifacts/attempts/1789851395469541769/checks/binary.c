...
static void binary_slice(void) {
 ...
 if(memcmp(saved_parent,parent,sizeof parent)) abort();memcpy(saved_children,s0,sizeof saved_children);
 LDL_dim2_fft3(d,tree.v,s0,s1,s0,8,0);
 if(memcmp(saved_children,s0,sizeof saved_children)) abort();checkmarks();
...
int main(int argc,char **argv) {
 ...
 if(!strcmp(argv[1],"frame")) {
  ...
  for(int k=-1;k<=1;k++) {
   ...
   if(ffLDL_inner_fft3(tree.v,a,b,a,8,tmp)!=2304||memcmp(expect,tree.v,sizeof expect)||memcmp(saved_children,a,sizeof saved_children)) abort();checkmarks();
  }
