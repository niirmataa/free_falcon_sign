...
def top {P B G C D E W : Type} (o : Ops P B G C D E W) (g : G) : Option (List W) := do
  ...
  let right←depth o d
  pure (z.1++left++right)
def rawPrefix {P B G C D E W : Type} (o : Ops P B G C D E W) (p : P) : Option (B×List W) := do
  let bg←o.basisGram p
  let tree←top o bg.2
  pure (bg.1,tree)
...
structure LocalDomains {P B G C D E W : Type} (o : Ops P B G C D E W) (Pkey : P → Prop) where
  ...
  eightGood : E → Prop
  sevenGood : E → Prop
  prefix_domain : ∀p,Pkey p → ∃bg,o.basisGram p=some bg ∧ rootGood bg.2
...
theorem inner_step_match {E W : Type} (o : Tower.Ops E W) (d : Nat)
    (e a b : E) (z : List W×E) (l r : List W)
    (ha : o.first e=some a) (hl : Tower.execute o d a=some l)
    (hz : o.ldl e=some z) (hb : o.second z.2=some b) (hr : Tower.execute o d b=some r) :
    Tower.execute o (d+1) e=some (z.1++l++r) := by
  simp [Tower.execute,ha,hl,hz,hb,hr]
theorem inner8_from_actual_inner7 {P B G C D E W : Type} (o : Ops P B G C D E W)
  ...
  exact ⟨z.1++l++r,inner_step_match o.binary 6 e a b z l r ha hl hz hb hr⟩
