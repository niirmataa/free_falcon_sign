...
(W/'artifacts/replay_evidence.json').write_text(json.dumps(dict(copied=len(rows),files=rows,semantic_matches=95,
 note='Verbatim full replay streams/receipts. Handwritten analytical proof is an input to review, not made kernel-certified by copying.'),indent=2)+'\n')
print(json.dumps(dict(archived_files=len(rows),semantic_matches=95)))
