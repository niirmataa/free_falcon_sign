...
theorem msb_and (a b : Nat) (ha : a<4294967296) (hb : b<4294967296) :
  ...
  have he : (a &&& b)/2147483648=(a/2147483648)&&&(b/2147483648) := by
    exact Nat.shiftRight_and_distrib (i:=31) (a:=a) (b:=b)
...
theorem none_word_spec (hi lo : Byte) (v : Int) (h : noneWord hi lo=some v) :
  ...
  unfold noneWord at h
  split at h
  · cases h
  · rename_i hn
    have hb:=none_flag_range _ (u32_range _) hn
    cases h
    have he : s16 (nonePost (noneRaw hi lo)-18433)=nonePost (noneRaw hi lo)-18433 :=
