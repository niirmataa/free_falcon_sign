"""Nested command receipts within the already write-isolated pipeline."""
...
def run(argv,label,expected=(0,),limit=60,extra=None):
    ...
    for key,data in [('stdout',out),('stderr',err)]:
        ...
        paths[key]=rel
    row=dict(start_utc=start,cwd=str(W),argv=argv,exit_code=p.returncode,timeout=timed,
             wall_limit_seconds=limit,elapsed_seconds=time.monotonic()-tick,**paths,
             stream_sha256={'stdout':hashlib.sha256(out).hexdigest(),'stderr':hashlib.sha256(err).hexdigest()},
             extra_environment=extra or {},expected_exit_codes=list(expected))
    ...
    if timed or p.returncode not in expected:
        print(out.decode(errors='replace'));print(err.decode(errors='replace'));raise RuntimeError('job failed: '+label)
    return row
