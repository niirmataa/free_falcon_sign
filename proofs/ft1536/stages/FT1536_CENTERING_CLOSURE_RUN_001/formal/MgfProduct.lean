...
-- Higiena instancji (jak przy SigmaMath.nonceFintype w pozostałych modułach):
-- boxVecFintype/boxPairFintype są choice'owe (Fintype.ofFinite), a sumy po
-- `Fin 768 → …` budują instancje piFinset — DEFEQ między nimi eksploduje
-- wykładniczo. Lokalnie zostają instancje strukturalne (pi/prod), więc
-- aplikacje generycznych lematów są syntaktyczne. Sumy pozostają tymi samymi
-- zbiorami (univ = univ), twierdzenia nie zależą od instancji.
...
/-- Rozkład sumy po parach na iloczyn dwóch potęg (poziom generyczny;
aplikacja po higienie instancji jest lustrzana). -/
theorem pair_sum_factor {D : Type*} [Fintype D] (φ : D → ℝ) (n : ℕ) :
    (∑ p : (Fin n → D) × (Fin n → D), (∏ i, φ (p.1 i)) * ∏ i, φ (p.2 i))
      = ((∑ d : D, φ d) ^ n) * ((∑ d : D, φ d) ^ n) := by
  have hinner2 : ∀ v : Fin n → D,
      (∏ i, φ (v i)) * (∑ w : Fin n → D, ∏ i, φ (w i))
        = (∑ w : Fin n → D, (∏ i, φ (v i)) * ∏ i, φ (w i)) :=
    fun v => Finset.mul_sum _ _ _
  calc (∑ p : (Fin n → D) × (Fin n → D), (∏ i, φ (p.1 i)) * ∏ i, φ (p.2 i))
      = ∑ v : Fin n → D, ∑ w : Fin n → D, (∏ i, φ (v i)) * ∏ i, φ (w i) := by
        rw [← Finset.univ_product_univ, Finset.sum_product]
    _ = ∑ v : Fin n → D, (∏ i, φ (v i)) * (∑ w : Fin n → D, ∏ i, φ (w i)) :=
        Finset.sum_congr rfl (fun v _ => (hinner2 v).symm)
    _ = (∑ v : Fin n → D, ∏ i, φ (v i)) * (∑ w : Fin n → D, ∏ i, φ (w i)) :=
        (Finset.sum_mul _ _ _).symm
    _ = ((∑ d : D, φ d) ^ n) * ((∑ d : D, φ d) ^ n) := by rw [sum_prod_fn φ n]
...
/-- Lemat 1: pełny rozkład MGF na 1536 czynników blokowych. -/
theorem mgf_product (s : ℝ) :
    ∑ z : BoxPair, Real.exp (-s * (Q (decode z) : ℝ)) = blockSum s ^ 1536 := by
  have hexp2prod : (∑ z : BoxPair, Real.exp (-s * (Q (decode z) : ℝ)))
      = ∑ z : BoxPair, (∏ i, slotVal s (z.1 i)) * ∏ i, slotVal s (z.2 i) :=
    Finset.sum_congr rfl (fun z _ => exp_Q_prod s z)
  have hdistr : (∑ z : BoxPair, (∏ i, slotVal s (z.1 i)) * ∏ i, slotVal s (z.2 i))
      = (blockSum s ^ 768) * (blockSum s ^ 768) :=
    pair_sum_factor (slotVal s) 768
  rw [hexp2prod, hdistr, ← pow_add]

/-- Wersja ważona, używana przez krok Chernoffa (`RejectionBound`). -/
theorem mgf_weighted (lam : ℝ) :
    (∑ z : BoxPair, gaussianWeight z * Real.exp (lam * (Q (decode z) : ℝ)))
      = blockSum (1/(2*768^2) - lam) ^ 1536 := by
  have h : ∀ z : BoxPair, gaussianWeight z * Real.exp (lam * (Q (decode z) : ℝ))
      = Real.exp (-(1/(2*768^2) - lam) * (Q (decode z) : ℝ)) := by
    intro z
    unfold gaussianWeight
    rw [← Real.exp_add]
    congr 1
    ring
  calc (∑ z : BoxPair, gaussianWeight z * Real.exp (lam * (Q (decode z) : ℝ)))
      = ∑ z : BoxPair, Real.exp (-(1/(2*768^2) - lam) * (Q (decode z) : ℝ)) :=
          Finset.sum_congr rfl (fun z _ => h z)
    _ = blockSum (1/(2*768^2) - lam) ^ 1536 := mgf_product _
