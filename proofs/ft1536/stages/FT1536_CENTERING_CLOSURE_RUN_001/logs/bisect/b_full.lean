...
/-! ## Premisa analityczna i kompozycja -/
...
/-- Kompozycja kernelowa: premisa analityczna + certyfikat numeryczny
ciągłego Chernoffa dają norm-reject pełnej wagi `≤ 2^-24`. -/
theorem full_rejection_bound (T : ThetaBounds)
    (hnum : Real.exp (-(lam * (B : ℝ))) * (Rrat ^ 1536)
      ≤ (1 / 2^24 : ℝ) * 9 / 10) :
    Rejection (fun _ : BoxPair => ()) () ≤ 1 / 2^24 := by
  have hbspos : ∀ s : ℝ, 0 < blockSum s := by
    intro s
    exact Finset.sum_pos' (fun d _ => (Real.exp_pos _).le)
      ⟨(0, 0), Finset.mem_univ _, Real.exp_pos _⟩
  have hpos : 0 < fiberMass (fun _ : BoxPair => ()) () := by
    rw [fiberMass_off]
    ...
    norm_num
  have hrw : Rejection (fun _ : BoxPair => ()) ()
      = fiberTailMass (fun _ : BoxPair => ()) ()
        / fiberMass (fun _ : BoxPair => ()) () := by
    unfold Rejection
    rw [trial_none_mass]
    split_ifs
  have hmk := fiberTailMass_le_exp (fun _ : BoxPair => ()) () lam lam_nonneg hpos
  rw [fiberTailMass_off, fiberMass_off, mass_off, mgf_on_off] at hmk
  have hU : blockSum sStar ^ 1536 ≤ Uint ^ 1536 :=
    pow_mono_nonneg _ _ (le_of_lt (hbspos sStar)) T.upper 1536
  ...
  have hUL : Uint / Lint = (c0 / sStar) * ((1 + eps) / (1 - eps)) := by
    ...
    field_simp
    ring
  have hcorr : ((1 + eps) / (1 - eps))^1536 ≤ 10 / 9 := by
  ...
  rw [hrw, fiberTailMass_off, fiberMass_off, mass_off]
  calc (∑ z, fiberWeight (fun _ : BoxPair => ()) () z
          * (if Q (decode z) < B then (0:ℝ) else 1))
        / blockSum c0 ^ 1536
      ≤ Real.exp (-(lam * (B : ℝ))) * blockSum sStar ^ 1536
          / blockSum c0 ^ 1536 := by
          exact hmk
    _ ≤ Real.exp (-(lam * (B : ℝ))) * Uint ^ 1536
          / blockSum c0 ^ 1536 := by
          refine div_le_div_of_nonneg_right ?_
            (pow_nonneg (le_of_lt (hbspos c0)) 1536)
          exact mul_le_mul_of_nonneg_left hU (Real.exp_nonneg _)
    _ ≤ Real.exp (-(lam * (B : ℝ))) * Uint ^ 1536
          / Lint ^ 1536 := by
          refine div_le_div_of_nonneg_left hL ?_
          exact mul_nonneg (Real.exp_nonneg _) (pow_nonneg (le_of_lt Uint_pos) 1536)
    _ = Real.exp (-(lam * (B : ℝ))) * (Uint ^ 1536 / Lint ^ 1536) := by
          rw [mul_div_assoc]
    _ = Real.exp (-(lam * (B : ℝ))) * ((c0 / sStar)
          * ((1 + eps) / (1 - eps))) ^ 1536 := by
          rw [← div_pow, hUL]
    _ = Real.exp (-(lam * (B : ℝ))) * (Rrat
          * ((1 + eps) / (1 - eps))) ^ 1536 := by
          rw [c0_div_sStar]
    ...
    _ ≤ ((1 / 2^24 : ℝ) * 9 / 10) * (10 / 9) := by
          refine mul_le_mul hnum hcorr ?_ ?_
          · exact pow_nonneg (div_nonneg (by nlinarith [eps_pos])
              (le_of_lt (by norm_num [eps] : (0:ℝ) < 1 - eps))) 1536
