...
set_option maxRecDepth 40000 in
/-- Krok B: ogon/masa ≤ exp(−λB)·(R·(1+eps)/(1−eps))^1536 — kompozycja
kroku Markowa (B1), lematu 1 i premisy theta; bez ∑-notacji. -/
  ...
  calc fiberTailMass (fun _ : BoxPair => ()) ()
    ...
    _ ≤ Real.exp (-lam * (B : ℝ)) * Uint ^ 1536
          ...
          refine div_le_div_of_nonneg_right ?_
            (pow_nonneg (le_of_lt (blockSum_pos c0)) 1536)
          exact mul_le_mul_of_nonneg_left hU (Real.exp_nonneg _)
    _ ≤ Real.exp (-lam * (B : ℝ)) * Uint ^ 1536
          / Lint ^ 1536 := by
          exact div_le_div_of_nonneg_left
            (mul_nonneg (Real.exp_nonneg _) (pow_nonneg (le_of_lt Uint_pos) 1536))
            (pow_pos Lint_pos 1536) hL
