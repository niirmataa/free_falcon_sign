...
theorem extract_congruence (h c s : Vec) : Congruent h c (Ext0Words h c s) := by
  intro i
  have general (x y : Int) : (FT1536.center (x-y)+y-x)%18433=0 := by
    have h:=center_congruence (x-y)
    omega
  exact general (c i) (product h s i)
...
theorem RAW_VERIFIER_SOUND (h c s : Vec) (hh : CanonVec h) (hc : CanonVec c) (hs : SignedVec s)
  ...
  change Q (Ext0Words h c s).1 s<2093922385
