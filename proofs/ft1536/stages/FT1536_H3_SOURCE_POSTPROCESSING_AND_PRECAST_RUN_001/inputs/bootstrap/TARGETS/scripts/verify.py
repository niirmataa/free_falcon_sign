...
def verify(W,pin):
 ...
 result=json.loads((W/'RESULT.json').read_text());assert result['status']=='H3_INITIAL_TARGETS_PROVED_FOR_EMITTED_PINNED_MODEL'
 return dict(status='PASS_FROZEN_PACKAGE',output_members=len(rows),output_bytes=sum(member(W,p).stat().st_size for p in rows),public_inputs=len(inputs),semantic_matches=len(expected),report_sha256=rows['REPORT.md'],outputs_sha256=pin,source_pin=result['source_pin'],certificate_sha256=rows['INITIAL_TARGET_CERTIFICATE.json'])
