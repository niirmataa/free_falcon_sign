...
Estar=2*(k0+k1)*Eterm
# The error is expressed in the (u,y) triangular coordinates, not raw (x,y).
# Bperp=B1-L_C*B0, and a>1/4, |L_C-c/a|<=32U*sqrt(j/a).
bperp=4*(q+dc)**2+(32*U)**2*jMax
assert aMax<2**23 and bperp<2**31
# Error in source-basis image: (delta_u-root_defect)B0+delta_y Bperp.
...
for pair,angle in zip(fixed,[pi/3,2*pi/3,4*pi/3]):
 for word,exact in zip(pair,[angle.cos(),angle.sin()]):assert abs(RBF(qv(word))-exact)<RBF(eps)
from backend import mul
double_inputs=[mul(a,b^(1<<63)) for a,b in cubic[512:768]]
assert all(1<=((word>>52)&2047)<2046 for word in double_inputs)
...
out=dict(schema='FT1536_POSTPROCESSING_NUMERIC_V1',status='PASS_COMPLETED_WHOLE_ROOT_SUFFIX_IFFT_RINT_DOMAINS',source_pin=sha(I/'CANDIDATE.sha256'),
 sampling_return=dict(domain='Actual whole ffSampling return in certified emitted/canonical/normalized/context domain; all terminal calls normal by LEFT',
  ...
  both_branch_metric_factors=[2,6],terminal_budget_each=str(Eterm),source_basis_root_transform_factors=[str(c0),str(c1)],
   exact_source_L_reference_image_energy=str(Estar),source_return_image_error_energy=str(Edelta),triangular_perpendicular_row_norm_squared_cap=str(bperp),right_only_energy_not_used_as_whole=True),
 ...
 ifft=dict(stages=stages,source_error_outward=str(bound),physical_index_stages=index_rows,twiddle_pairs_checked=tw,
  inverse768_bits=f'{ni:016x}',terminal_scalar_error=[str(e0),str(e1)],exact_inverse_identity='cubic inverse*forward=3I; each binary=2I; terminal real fold inverse; scale1/768',
   rounded_algorithm_not_assumed_linear=True,all_stage_source_operands_below='2^40',source_output_abs_cap=str(floatcap),fixed_components_checked=6,squared_twiddle_double_inputs_checked=256),
