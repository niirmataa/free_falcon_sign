...
W=Path.cwd();D=W/'tmp/rejected_wrong_external_pin';assert not D.exists();cmd=[sys.executable,'-B','scripts/replay.py',str(D),'0'*64,'--anchor'];p=subprocess.run(cmd,capture_output=True,timeout=30)
for s,b in [('stdout',p.stdout),('stderr',p.stderr)]:(W/'logs'/('replay_wrong_pin.'+s)).write_bytes(b)
