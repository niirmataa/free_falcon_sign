...
as_gib=int(os.environ.get('L_GLOBAL_AS_GIB','8'));assert 1<=as_gib<=16
...
def bounds():
    ...
    # Lean/model checks have a finite address-space budget; C ASan is a separate
    # mode because its shadow mapping intentionally needs much more virtual space.
    if not os.environ.get('L_GLOBAL_ASAN_JOB'):resource.setrlimit(resource.RLIMIT_AS,(as_gib*1024**3,as_gib*1024**3))
...
with (W/'executor.lock').open('a') as lock:
    ...
    row=dict(start_utc=start,cwd=str(W),argv=argv,sandbox=box,exit_code=p.returncode,timeout=timed,
       wall_limit_seconds=limit,cpu_limit_seconds=limit,address_space_bytes=None if os.environ.get('L_GLOBAL_ASAN_JOB') else as_gib*1024**3,
