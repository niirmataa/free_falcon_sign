...
def snapshot (a : Array Int) : Mem := fun i => a[i]!
-- Execute the imported step on a materialized pre-state, then materialize
-- its two/three changed cells. Buffer.pair_frame/triple_frame justify keeping
-- all other cells. This avoids an exponentially re-evaluated closure chain.
def pairs (out : IO.FS.Handle) (phase : String) (m h : Nat)
    (op : Nat → (Bool → Int) → Bool → Int) (input : Array Int) : IO (Array Int) := do
  let mut s:=input
  for b in [:768] do
    let next:=pairStep (pairAddr h) op b (snapshot s)
    let lo:=pairAddr h b false;let hi:=pairAddr h b true
    s:=(s.set! lo (next lo)).set! hi (next hi)
    emit out phase m h (b+1) (snapshot s)
  return s
def triples (out : IO.FS.Handle) (phase : String)
    (op : Nat → (Fin 3 → Int) → Fin 3 → Int) (input : Array Int) : IO (Array Int) := do
  let mut s:=input
  for b in [:512] do
    let next:=tripleStep tripleAddr op b (snapshot s)
    s:=((s.set! (3*b) (next (3*b))).set! (3*b+1) (next (3*b+1))).set! (3*b+2) (next (3*b+2))
    emit out phase 0 0 (b+1) (snapshot s)
  return s
def main (args : List String) : IO Unit := do
  ...
  let mut s:=input
  emit out "input" 0 0 0 (snapshot s)
  ...
  emit out "forward" 0 0 1536 (snapshot s)
  ...
  for i in [:1536] do
    let next:=scaleStep i (snapshot s)
    s:=s.set! i (next i)
    emit out "scale" 0 0 (i+1) (snapshot s)
  emit out "roundtrip" 0 0 1536 (snapshot s)
  out.flush
