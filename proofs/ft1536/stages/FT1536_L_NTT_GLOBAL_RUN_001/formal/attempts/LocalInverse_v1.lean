...
theorem local_at (i : Nat) (hi0 : 0<i) (hi : i<1024) :
  ...
  have hn : rowID (unitRow ⟨i,hi⟩)≠0 := by rw [unitRow_id]; omega
  simpa only [localCheck,if_neg hn,Bool.and_eq_true,gmAt,igmAt,dif_pos hi,gm,igm] using h
...
theorem cubic_homogeneous (b : Nat) (c : Int) (v : Fin 3 → Int) (hv : ∀ k, CanonVal (v k)) (k : Fin 3) :
  ...
  simpa only [cubeI_bind,scaled3] using eval_scaled (cubeIExpr (igmAt (512+b)) iW k) c v hv
