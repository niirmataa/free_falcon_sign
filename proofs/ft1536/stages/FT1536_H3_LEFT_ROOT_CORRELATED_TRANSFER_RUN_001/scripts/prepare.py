...
W=Path.cwd();I=W/'inputs/bootstrap';TASK=Path('/home/footfalcon/Dokumenty/FT1536_ZADANIE_ASTRA_H3_LEFT_ROOT_CORRELATED_TRANSFER_2026-09-20.md')
def sha(p):
 with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
assert W==Path('/home/footfalcon/free_falcon_sign/proofs/ft1536/work/FT1536_H3_LEFT_ROOT_CORRELATED_TRANSFER_RUN_001')
assert sha(TASK)=='6e06bfd615178873a081c369ae8b89a4ab1203e73044a0cc8839d137f26ef72e'
assert sha(I/'MANIFEST.sha256')=='5bbd7b14d05275ad0cf46a72e9b8d24d5c67001f7597761edb07d86d444173d4'
rows={}
...
assert len(rows)==579 and {p.relative_to(I).as_posix() for p in I.rglob('*') if p.is_file()}==set(rows)|{'MANIFEST.sha256'}
orig=json.loads((I/'ORIGINS.json').read_text());assert len(orig['files'])==577 and orig['base_commit']=='7664277d6da6839973e8a00db5b20c834bb63adc'
for r in orig['files']:assert rows[r['copy']]==r['sha256'] and (I/r['copy']).stat().st_size==r['bytes']
assert sum((I/r).stat().st_size for r in rows)==9599951
...
for n in ['lean','dyadic','fp_literal','replaylib','literal_backend','half_model','root_model','backend','toolchain','ordered_model']:
 src=I/'ORDERED/scripts'/(n+'.py');dst=W/'scripts'/(n+'.py');shutil.copyfile(src,dst);reuse.append(dict(input=src.relative_to(W).as_posix(),copy=dst.relative_to(W).as_posix(),sha256=sha(dst),byte_identical=True))
order=[]
def visit(m):
 if m in order:return
 src=I/'ORDERED/formal'/(m+'.lean');assert src.is_file()
 for d in re.findall(r'^import (\w+)',src.read_text(),re.M):
  if (I/'ORDERED/formal'/(d+'.lean')).exists():visit(d)
...
out=dict(status='PASS',members=579,origins=577,source_files=17,public_inputs=len(prov),member_bytes=9599951,all_pins_match=True,exact_scope=True,source_changed=False,inherited_modules=order)
