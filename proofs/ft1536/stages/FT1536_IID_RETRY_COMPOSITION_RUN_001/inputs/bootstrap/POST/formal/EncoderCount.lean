...
theorem drain_ok (fuel : Nat) (cap : Int) (s : Count)
  ...
  | succ k ih =>
    ...
    · have hc : s.used<cap := by unfold weight at budget;omega
      have he : weight ⟨s.used+1,s.bits-8⟩=weight s := by dsimp [weight];omega
...
theorem prefix9_ok (cap : Int) (s : Count) (hs : CountInv s) (budget : weight s+9≤8*cap) :
  ...
  have he : weight ⟨s.used,s.bits+9⟩=weight s+9 := by dsimp [weight];omega
...
theorem STATIC_FITS_4096 (a b : Vec) (ha : SignedVec a) (hb : SignedVec b) (accept : isShort a b=true) :
  ...
  rw [hbits] at he
  refine ⟨?_,hcap.2⟩
  have hlen : payloadLength b-1=(staticBits b+7)/8 := by unfold payloadLength;omega
  rw [hlen]
  exact he.1
...
theorem post_decrement_count (m t : Nat) (hm : m≤128) (ht : t≤m) :
    0≤(m:Int)-(t:Int) ∧ -1≤(m:Int)-(t:Int)-1 ∧ (m:Int)-(t:Int)-1≤127 ∧
    (m:Int)-((m:Int)+1)= -1 ∧ (m:Int)-((m:Int)+1)-1= -2 := by omega
...
#print axioms post_decrement_count
end FT1536M0
