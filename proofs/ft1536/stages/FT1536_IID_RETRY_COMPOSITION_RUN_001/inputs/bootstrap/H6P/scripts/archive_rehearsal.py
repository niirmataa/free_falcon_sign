...
paths +=[D/'artifacts'/p for p in ['kernel.jsonl','final_kernel_receipts.json','build_normal.json','build_sanitized.json','toolchain_commands.json']]+list((D/'artifacts').glob('native_receipts_*.json'));rows=[]
...
shutil.copyfile(D/'REPLAY_RESULT.json',W/'artifacts/fresh_replay.json');(W/'artifacts/rehearsal_evidence.json').write_text(json.dumps(dict(status='PASS_ARCHIVED_FRESH_REHEARSAL',files=rows,matched=result['matched'],expected=result['expected_files'],external_anchor_sha256=result['external_manifest_sha256'],sealed_receipt_sha256=sha(W/'artifacts/fresh_replay.json')),indent=2)+'\n');print('ARCHIVED_FRESH_REPLAY',len(rows),'matches',result['matched'])
