...
theorem RHO_CLOSED (x : Word) (hx : NumericCenter x) : 0≤rhoNum x ∧ rhoNum x≤D := by
  ...
  · rw [eps0,ite_eq_right hz,Int.sub_zero]
    ...
    have hp:=Int.emod_nonneg (valueNum x) (show D≠0 by have hd:=D_pos;omega)
...
theorem old_implies_numeric (x : Word) (h : H3Range.CenterClass x.val) : NumericCenter x := by
  have he : ex x≤1053 := h.2.1
  have hf : -2147483283≤floorVal x ∧ floorVal x≤2147483281 := by
    rw [floorVal_old x he]
    exact h.2.2.2
  refine ⟨by omega,?_,?_⟩
  · exact (Int.le_ediv_iff_mul_le D_pos).mp hf.1
  · apply (Int.ediv_lt_iff_lt_mul D_pos).mp
    change floorVal x<2147483282
    omega
...
#print axioms old_implies_numeric
end ZeroScalar
