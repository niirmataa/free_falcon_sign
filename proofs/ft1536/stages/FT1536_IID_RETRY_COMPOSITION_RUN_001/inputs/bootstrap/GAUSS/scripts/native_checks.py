...
for c in cases:
 assert sha(W/c['input'])==c['input_sha256'] and sha(W/c['expected'])==c['expected_sha256'];cmd=['bin/gaussian-'+mode,c['mode']];t=time.monotonic();p=subprocess.run(cmd,input=(W/c['input']).read_bytes(),capture_output=True,timeout=60);tag='native_'+mode+'_'+c['name']
...
(W/'artifacts'/('native_'+mode+'_'+str(group)+'.json')).write_text(json.dumps(dict(status='PASS_ORIGINAL_C_WORDS',mode=mode,group=group,matches=matches),indent=2)+'\n')
