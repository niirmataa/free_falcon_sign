...
inductive ZeroPrefix (d : Bytes) : Nat → Cursor → Cursor → Prop where
  | nil (c) : ZeroPrefix d 0 c c
  | cons (k c r t) (h : nextBit d c=some (0,r)) (tail : ZeroPrefix d k r t) : ZeroPrefix d (k+1) c t
theorem zeroCount_shift (k ne : Nat) (hne : ne<4294967296) :
    zeroCount k ((ne+1)%4294967296)=zeroCount (k+1) ne := by
  rw [unary_wrap k _ (Nat.mod_lt _ (by decide)),unary_wrap (k+1) ne hne]
  omega
theorem unary_skip_zeros (d : Bytes) (k : Nat) (c r : Cursor) (hpref : ZeroPrefix d k c r)
    (fuel ne : Nat) (hne : ne<4294967296) :
    unaryFuel (k+fuel) d ne c=unaryFuel fuel d (zeroCount k ne) r := by
  induction hpref generalizing ne with
  | nil c => simp only [Nat.zero_add,zeroCount]
  | cons k c r t h tail ih =>
    have he : k+1+fuel=(k+fuel)+1 := by omega
    rw [he,unaryFuel,h]
    dsimp only
    rw [ite_eq_left rfl,ih ((ne+1)%4294967296) (Nat.mod_lt _ (by decide)),zeroCount_shift k ne hne]
theorem unary_full_wrap : zeroCount 4294967296 0=0 ∧
    zeroCount 4294967551 0=255 ∧ zeroCount 4294967552 0=256 := by
  rw [unary_wrap _ 0 (by decide),unary_wrap _ 0 (by decide),unary_wrap _ 0 (by decide)]
  decide
...
#print axioms zeroCount_shift
#print axioms unary_skip_zeros
#print axioms unary_full_wrap
end FT1536Bridge
