...
paths=['COMMANDS.log','REPLAY_RESULT.json','artifacts/kernel.jsonl','artifacts/final_kernel_receipts.json','artifacts/toolchain_commands.json','artifacts/preprocessed/commands.json','artifacts/lean_values_command.json','checks/logs/normal/commands.json','checks/logs/san/commands.json']
paths+=sorted(p.relative_to(D).as_posix() for folder in ['logs','artifacts/replay_jobs'] for p in (D/folder).rglob('*') if p.is_file())
for name in paths:
