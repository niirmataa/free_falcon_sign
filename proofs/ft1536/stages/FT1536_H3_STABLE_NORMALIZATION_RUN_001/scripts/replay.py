...
def job(tag,seconds,script,*arguments,asan=False,sage=False):
 ...
 out=D/'artifacts/replay_jobs';so=out/(tag+'.stdout');se=out/(tag+'.stderr');start=time.monotonic();timed=False
 ...
 r['jobs'].append(row);save();print(json.dumps(dict(job=tag,exit_code=p.returncode,elapsed_seconds=row['elapsed_seconds'])),flush=True);assert not timed and p.returncode==0,tag
try:
 save();job('toolchain',60,'toolchain');job('binding',120,'source_binding');job('source_checks',60,'source_checks')
 job('width_bounds',120,'width_bounds',sage=True);job('kernel',240,'kernel_build');job('audit',90,'audit')
 job('fixtures',240,'fixtures');job('lean_values',120,'lean_values');job('normal',120,'controls','normal');job('sanitizers',120,'controls','san',asan=True)
 job('map',60,'map_certificate');job('mutations',180,'mutations');job('certificate',60,'certificate')
