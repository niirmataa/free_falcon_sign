...
W=Path.cwd();I=W/'inputs/bootstrap';TASK=Path('/home/footfalcon/Dokumenty/FT1536_ZADANIE_ASTRA_H3_STABLE_NORMALIZATION_2026-09-20.md')
def sha(p):
 with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
assert W==Path('/home/footfalcon/free_falcon_sign/proofs/ft1536/work/FT1536_H3_STABLE_NORMALIZATION_RUN_001')
assert sha(TASK)=='feba3eed88442f7259f1be6283b2964c19065c7984611df2ea1af054da5cab7d'
assert sha(I/'MANIFEST.sha256')=='58f027900b3283bdf6f488ddb4fdeaa1e6c80b69253cf7d03a7855d3898478f7'
...
orig=json.loads((I/'ORIGINS.json').read_text());assert len(orig['files'])==270 and orig['base_commit']=='a53d7231795fb5c7317bc836baaf0ccd3ec2652f'
for r in orig['files']:assert rows[r['copy']]==r['sha256'] and (I/r['copy']).stat().st_size==r['bytes']
assert sum((I/r).stat().st_size for r in rows)==6309831
...
reuse=[]
for n in ['lean','dyadic','fp_literal','replaylib','literal_backend','half_model','root_model']:
 src=I/'RAW/scripts'/(n+'.py');dst=W/'scripts'/(n+'.py');shutil.copyfile(src,dst);reuse.append(dict(input=src.relative_to(W).as_posix(),copy=dst.relative_to(W).as_posix(),sha256=sha(dst),byte_identical=True))
for src in sorted((I/'RAW/formal').glob('*.lean')):
...
diff=''.join(difflib.unified_diff((I/'RAW/scripts/run.py').read_text().splitlines(True),(W/'scripts/run.py').read_text().splitlines(True),fromfile='inputs/bootstrap/RAW/scripts/run.py',tofile='scripts/run.py'))
...
out=dict(status='PASS',members=272,origins=270,source_files=17,public_inputs=len(provenance),member_bytes=6309831,all_pins_match=True,exact_scope=True,source_changed=False)
