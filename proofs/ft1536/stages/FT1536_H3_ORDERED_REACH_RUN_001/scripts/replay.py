...
r=dict(status='RUNNING',claim_status='PARTIAL_PROOF',mode='PREFREEZE_REHEARSAL' if args.rehearsal else 'STANDARD',input_manifest=manifest,input_manifest_sha256=args.external_sha256,source=str(W),destination=str(D),seed_members=len(seed),cached_olean_or_binaries_used=False,source_changed=False,jobs=[]);tick=time.monotonic()
...
try:
 save();job('toolchain',60,'toolchain');job('binding',120,'source_binding');job('source_checks',60,'source_checks');job('bounds',60,'bounds');job('scalar_contract',60,'scalar_contract')
 job('transfer',120,'transfer_certificate',sage=True);job('kernel',240,'kernel_build');job('audit',90,'audit');job('fixtures',240,'fixtures')
 job('normal',180,'controls','normal');job('sanitizers',180,'controls','san',asan=True);job('scalar_normal',120,'scalar_controls','normal');job('scalar_san',120,'scalar_controls','san',asan=True)
 job('mutations',240,'mutations');job('certificate',60,'certificate')
