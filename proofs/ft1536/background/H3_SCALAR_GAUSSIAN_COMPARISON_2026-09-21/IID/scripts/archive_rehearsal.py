...
paths += [D/'artifacts'/p for p in ['kernel.jsonl','final_kernel_receipts.json','build_normal.json','build_sanitized.json','toolchain_commands.json']]+list((D/'artifacts').glob('native_receipts_*.json'))
rows=[]
...
# Sealed historical receipt in precisely the importer-friendly requested format.
