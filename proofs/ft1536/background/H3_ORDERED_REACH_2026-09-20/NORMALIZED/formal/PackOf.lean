  ...
  · have hp : 0<i.natAbs := by omega
    ...
    have rm : roundMant (sticky (norm64 i.natAbs).1 9)=i.natAbs*2^((norm64 i.natAbs).2-11) := by
      ...
      have h8 : (i.natAbs*(2^((norm64 i.natAbs).2-11)*2^2))%8=0 := by
        ...
        rw [hex2,Nat.pow_add]
        rw [Nat.mul_assoc (2^((norm64 i.natAbs).2-12)) (2^1) (2^2),←Nat.mul_assoc]
        simp
      unfold roundMant
      rw [h8]
      dsimp
      rw [←Nat.mul_assoc,Nat.mul_div_cancel _ (by decide)]
    ...
    · rename_i hnot
      have hnneg : 0≤i := by simp only [decide_eq_true_eq] at hnot;omega
