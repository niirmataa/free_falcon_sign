"""Logged bounded jobs: only W writable; bootstrap and source stay read-only."""
...
env=dict(os.environ);env.update(HOME=str(W/'cache/home'),TMPDIR=str(W/'tmp'),TMP=str(W/'tmp'),TEMP=str(W/'tmp'),
 ...
 PYTHONDONTWRITEBYTECODE='1',PYTHONOPTIMIZE='0',MAMBA_ROOT_PREFIX='/home/footfalcon/miniforge3',
...
for rel in ['source','inputs/bootstrap']:
...
with (W/'executor.lock').open('a') as lock:
    ...
    row=dict(start_utc=start,cwd=str(W),argv=argv,sandbox=box,exit_code=p.returncode,timeout=timed,
      wall_limit_seconds=limit,cpu_limit_seconds=limit,address_space_bytes=None if asan else 8*1024**3,asan_shadow_reservation=asan,
