...
- **H-B (σ_N i margines) — WYCOFANA w pierwotnym kształcie po teście
  ...
  Niezależny rachunek przedziałowy (256-bit, odbiór):
  `Pr[Q ≥ B] = [2.9925420736…e−9 ± 2.32e−82] = 2^−28.32…`, czyli
  **> 2^−40 — kryterium „≥ 1 − 2^−40" jest w tym modelu spełnione z ujemnym
  zapasem (nie jest spełnione)**. To ujemny wynik dla hipotezy w modelu
  ciągłym — nie atak, nie prawo Sign po castach (M3 otwarte).
  **Przyjęte kryterium (decyzja właściciela: „zgodne z modelem"):** masa
  akceptacji na poziomie modelu, tzn. ogon idealny `≤ 2^−28` na próbę —
  obecny margines spełnia z zapasem 0.3 bitu (2^−28.32); prawdziwe prawo po
  castach/retry pozostaje osobnym obowiązkiem. Margines 1.075 roli
  „tailcut_rate" z konwencji Falcon v1.2 (tam 1.1) — mapowanie jawne.
