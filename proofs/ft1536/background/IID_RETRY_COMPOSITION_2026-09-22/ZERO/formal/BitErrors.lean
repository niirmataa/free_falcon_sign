...
theorem sticky_interval (x k : Nat) :
  ...
  have hd:=Nat.two_pow_pos k
  have hre : x%2^k+x/2^k*2^k=x := by simpa only [Nat.mul_comm] using Nat.mod_add_div x (2^k)
...
theorem normalizer_for_add (t : Nat) (ht : 0<t ∧ t<144115188075855872) :
    7≤(norm64 t).2 ∧ (norm64 t).2≤63 := by
  have h:=normalizer_bounds t (by omega)
  refine ⟨?_,h.1⟩
  by_cases hs : 7≤(norm64 t).2
  · exact hs
  · have hp : 2^(norm64 t).2≤64 := Nat.pow_le_pow_right (by decide : 0<2) (show (norm64 t).2≤6 by omega)
    have hm:=Nat.mul_le_mul (show t≤144115188075855871 by omega) hp
    omega
theorem shrink9_range (n : Nat) (hn : 9223372036854775808≤n ∧ n<18446744073709551616) :
    18014398509481984≤sticky n 9 ∧ sticky n 9<36028797018963968 := by
  have hr : n%512+511<18446744073709551616 := by omega
  have hu : (n ||| (n%512+511))<18446744073709551616 := Nat.or_lt_two_pow (n:=64) hn.2 hr
  have hl : n≤(n ||| (n%512+511)) := Nat.left_le_or
  change 18014398509481984≤(n ||| (n%512+511))/512 ∧ (n ||| (n%512+511))/512<36028797018963968
  omega
...
#print axioms normalizer_for_add
#print axioms shrink9_range
end ZeroScalar
