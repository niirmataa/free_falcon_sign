import json,shlex,subprocess,time,sys
from pathlib import Path
from replaylib import sha
W=Path.cwd();mode=sys.argv[1];assert mode in ['normal','sanitized']
flags=next(s.split('=',1)[1].strip() for s in (W/'source/Makefile').read_text().splitlines() if s.startswith('CFLAGS ='))
flags=shlex.split(flags)
# shlex retains the literal quotes escaped by the Makefile for the shell.
assert '-O' in flags and '-DFPR_IMPL="fpr-emulated.h"' in flags
cmd=['gcc','-std=c99']+flags+['-Isource','checks/post.c','source/fpr-emulated.c','source/shake.c','-lm','-o','bin/post-'+mode]
...
for s,d in [('stdout',p.stdout),('stderr',p.stderr)]:(W/'logs'/('build_'+mode+'.'+s)).write_bytes(d)
