...
normerr=3*U*16+128*eta
assert (2*U+U*U)*16+(3+2*U)*eta<normerr
 ...
 for level in range(8,0,-1):
  ...
  for r in rs:
   ...
   local.append(dict(branch=b,level=level,diagonal=r['diagonal'],path=r['path'],m=str(m),imag=str(im),split_error=str(delta),H_lower=str(lam),h_upper=str(hmax),split_relative=str(rho),factor_relative=str(ldl),lower=str(a),upper=str(z),
    source_PC='ffLDL_inner_fft3 local620 or641; SplitDeep637/643; original raw construction snapshot',domain='Actual P_key-derived parent complex diagonal and local source H; TOWER record m/M/I/delta',
    dependencies=['TOWER/artifacts/numeric_certificate.json','source dim2 reconstruction residual lemma','signed real-projection split bound'],discharge='PROVED_LOCAL_SOURCE_FACTOR_COMPARISON'))
