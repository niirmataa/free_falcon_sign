...
def verify(W,pin):
 ...
 assert fresh['status']=='FRESH_REPLAY_PASS' and expected==fresh['matches']
 assert set(semantic_paths(W))=={r['path'] for r in expected}
 ...
 result=json.loads((W/'RESULT.json').read_text());assert result['status']=='H3_RAW_ASSEMBLY_PROVED_FOR_PINNED_MODEL'
 return dict(status='PASS_FROZEN_PACKAGE',output_members=len(rows),output_bytes=sum(member(W,p).stat().st_size for p in rows),public_inputs=len(inputs),
  semantic_matches=len(expected),report_sha256=rows['REPORT.md'],outputs_sha256=pin,source_manifest_sha256=result['source_manifest_sha256'],raw_certificate_sha256=rows['RAW_PREFIX_CERTIFICATE.json'])
