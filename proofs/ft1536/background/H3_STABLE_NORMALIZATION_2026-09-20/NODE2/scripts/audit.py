...
W=Path.cwd();old=json.loads((W/'artifacts/inherited_order.json').read_text());new=['Half','BinaryMargin','BinaryFrame','BinaryConstants','HalfMain'];mods=old+new
...
audit='import BinaryConstants\nimport HalfMain\nset_option pp.universes true\n'+'\n'.join('#print axioms '+n for n in decls)+'\n'
types='import BinaryConstants\nimport HalfMain\nset_option pp.all true\n'+'\n'.join('#check @'+n+'\n#print '+n for n in newdecls)+'\n'
types+='\n'.join('#print Node2.'+n for n in ['halfC','inputNum','halfNum','c2','Valid','c00','c01','c02','c10','c11','c12'])+'\n'
for n,data in [('BinaryAudit',audit),('BinaryTypes',types)]:
 p=W/'formal'/(n+'.lean')
 if p.exists():assert p.read_text()==data
 else:p.write_text(data)
 sources[str(p.relative_to(W))]=sha(p)
p=subprocess.run([sys.executable,'-B','scripts/lean.py','BinaryAudit','BinaryTypes'],capture_output=True,timeout=60)
...
for mod in mods+['BinaryAudit','BinaryTypes']:
 ...
 receipts.append(r)
text=(final/'BinaryAudit.stdout').read_text();allowed={'propext','Classical.choice','Quot.sound'}
...
I=W/'inputs/bootstrap';boot=verify_manifest(I,'MANIFEST.sha256','f92dbaa6f4262f2cce5d4bd27e002da0f684f87a92fef8619bee226221782178');assert len(boot)==120
...
for mod in old:assert sha(W/'formal'/(mod+'.lean'))==boot['NODE3/formal/'+mod+'.lean']
for line in (I/'CANDIDATE.sha256').read_text().splitlines():
 h,n=line.split('  ',1);assert sha(W/'source'/n)==h
out=dict(status='PASS_KERNEL_HALF_AND_LEMMAS_WITH_PROVENANCE',modules=len(mods)+2,checked_theorems=len(decls),new_theorems=len(newdecls),
 ...
 audit_stdout_sha256=sha(final/'BinaryAudit.stdout'),full_types_terms_sha256=sha(final/'BinaryTypes.stdout'),
 analytical_proofs={r:sha(W/r) for r in ['ANALYTIC_PROOF.md','UPSTREAM_REFINEMENT.md','NODE2_FRAME.md']},
 full_node2_theorem_kernelized=False,proof_layer='Kernel half bit/value, pair/constant/frame lemmas; universal upstream and complex source composition analytical with exact certificates.',
 level=dict(split_logn=9,ldl_logn=8,full=0),H3_range_proved=False,full_internal_tree_proved=False,global_reachability_proved=False,sampler_law_proved=False)
