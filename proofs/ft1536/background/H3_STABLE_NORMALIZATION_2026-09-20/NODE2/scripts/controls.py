...
for b in [0,1]:
 ...
 for k,idx in enumerate([0,3,4]):
  ...
  parentraw=run(f'parent_{b}_{k}',[exe,'parents',str(k)],text.encode())
  assert [int(x,16) for x in parentraw.split()]==v
  (W/'checks'/f'parent_{b}_{k}.txt').write_bytes(parentraw)
...
# A legal numerical-superset input whose half creates a subnormal s0 imaginary word.
subcase=[rn(1)]*256+[0x0010000000000001 if i%2==0 else 0 for i in range(256)]
submodel=slice_words(subcase,tab);payload=''.join(f'{x:016x}\n' for x in subcase).encode()
(W/'checks/subnormal_input.txt').write_bytes(payload);subraw=run('subnormal_split',[exe,'direct'],payload)
(W/'checks'/f'subnormal_{mode}.txt').write_bytes(subraw)
assert [tuple(int(z,16) for z in l.split()) for l in subraw.decode().splitlines()]==list(zip(*submodel))
assert submodel[0][128]==1 and submodel[3][128]==0
stops=[]
...
out=dict(status='PASS_FINITE_BINDING_CONTROLS',level=dict(split_logn=9,ldl_logn=8,full=0),cases=summaries,half_words=len(words),
 ...
 subnormal_split_control=dict(positions=128,s0_imag_bits='0000000000000001',d11_imag_bits='0000000000000000',actual_C_and_model_equal=True),
