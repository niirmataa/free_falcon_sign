...
phase=[CB((2*pi*r/4608).cos(),-(2*pi*r/4608).sin()) for r in range(4608)];s3=RB(3).sqrt()
def direct(words,source,name):
 accum=[CB(0) for _ in range(768)]
 for j in range(768):
  y=CB(rat(words[j]),rat(words[j+768]))
  for k in range(768):accum[k]+=y*phase[(roots[j]*k)%4608]
 accum=[z/768 for z in accum];ideal=[z.real()-z.imag()/s3 for z in accum]+[2*z.imag()/s3 for z in accum]
 errors=[abs(RB(rat(w))-x) for w,x in zip(source,ideal)]
 failures=[dict(i=i,source=str(rat(source[i])),ideal=str(ideal[i]),error=str(e)) for i,e in enumerate(errors) if not e<RB(QQ(1)/128)]
 if failures:
  (W/'artifacts'/('oracle_failure_'+name+'.json')).write_text(json.dumps(failures,indent=2)+'\n');print(json.dumps(failures[:8],indent=2),flush=True)
 assert not failures
...
for c in cases:
 if len(sys.argv)>1 and c['name']!=sys.argv[1]:continue
