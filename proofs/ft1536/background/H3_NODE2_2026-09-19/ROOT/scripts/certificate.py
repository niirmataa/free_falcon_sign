...
assert (QQ(1)/2-8*eta)/(1+4*u)-da>QQ(1)/4
...
assert (1+gamma)*2*(N+df)*(H+dF)<2**35 and (1+gamma)*jhatmax<2**45 and (1-gamma)/4>QQ(1)/8
out=dict(schema='H3_ROOT_NUMERIC_CERTIFICATE_V1',status='PASS_EXACT_COMPOSITION',
 ...
 constants=dict(fft_component_error_fg=str(ef),fft_component_error_FG=str(eG),gram_error_A='1/1024',gram_error_C_norm='1',gram_error_J='1024',
  fft_component_abs_fg=str(N+ef),fft_component_abs_FG=str(H+eG),source_g10_norm_upper='34359738368',source_g11_real_lower='1/8',source_g11_real_upper='35184372088832',
