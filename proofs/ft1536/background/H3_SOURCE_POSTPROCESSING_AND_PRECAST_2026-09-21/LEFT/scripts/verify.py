...
def verify(W,pin):
 ...
 p=json.loads((W/'artifacts/commands_prefix.json').read_text());data=(W/p['path']).read_bytes();assert len(data)==p['bytes'] and len(data.splitlines())==p['records'] and sha(W/p['path'])==p['sha256']
 ...
 result=json.loads((W/'RESULT.json').read_text());assert result['status']=='H3_LEFT_ROOT_CORRELATED_TRANSFER_PROVED_FOR_EMITTED_PINNED_MODEL' and result['ordered_root_reach_proved']
 return dict(status='PASS_FROZEN_PACKAGE',output_members=len(rows),output_bytes=sum(member(W,p).stat().st_size for p in rows),public_inputs=len(inputs),semantic_matches=len(expected),report_sha256=rows['REPORT.md'],outputs_sha256=pin,source_pin=result['source_pin'],certificate_sha256=rows['LEFT_ROOT_CORRELATED_TRANSFER_CERTIFICATE.json'])
