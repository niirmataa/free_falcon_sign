...
theorem square_nonnegative (x : Int) : 0≤x*x := by
  ...
  · have hp:=Int.mul_nonneg (show 0≤ -x by omega) (show 0≤ -x by omega)
    simpa only [Int.neg_mul_neg] using hp
theorem a2_square_bound (x y : Int) : x*x+y*y≤2*A2 x y := by
  have h:=square_nonnegative (x+y)
...
theorem abs_majorant (x : Int) : 3302*absI x≤x*x+2725801 := by
  have h:=square_nonnegative (absI x-1651)
