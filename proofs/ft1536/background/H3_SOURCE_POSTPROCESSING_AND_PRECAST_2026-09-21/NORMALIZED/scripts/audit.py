...
for p in sorted((W/'formal').glob('*.lean')):
 if p.stem not in ['RawAudit','RawTypes','StableAudit','StableTypes','SqrtRunnerTypes']:visit(p.stem)
...
for name,text in [('StableAudit',a),('StableTypes',t),('SqrtRunnerTypes','import SqrtMain\nset_option pp.all true\n#check @main\n#print main\n')]:
 p=W/'formal'/(name+'.lean')
 if p.exists():assert p.read_text()==text
 else:p.write_text(text)
 sources[p.relative_to(W).as_posix()]=sha(p)
p=subprocess.run([sys.executable,'-B','scripts/lean.py','StableAudit','StableTypes','SqrtRunnerTypes'],capture_output=True,timeout=60)
...
for m in mods+['StableAudit','StableTypes','SqrtRunnerTypes']:
...
out=dict(status='PASS_KERNEL_SQRT54_BIT_BRIDGE_GATE_MAPPING_AND_SCOPED_CONSUMERS',modules=len(mods)+3,checked_theorems=len(decls),new_theorems=len(fresh),
 ...
 audit_stdout_sha256=sha(final/'StableAudit.stdout'),types_terms_stdout_sha256=sha(final/'StableTypes.stdout'),standalone_IO_types_sha256=sha(final/'SqrtRunnerTypes.stdout'),
