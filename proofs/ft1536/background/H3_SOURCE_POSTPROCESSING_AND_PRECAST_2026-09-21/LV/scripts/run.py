"""Bounded, logged jobs; only this W is writable, source/ read-only."""
...
for p in ['logs','artifacts','tmp','cache/home','cache/sage','cache/mpl','cache/ipython']:(W/p).mkdir(parents=True,exist_ok=True)
env=dict(os.environ);env.update(HOME=str(W/'cache/home'),TMPDIR=str(W/'tmp'),TMP=str(W/'tmp'),TEMP=str(W/'tmp'),
 ...
 PYTHONDONTWRITEBYTECODE='1',PYTHONOPTIMIZE='0',PYTHONPYCACHEPREFIX=str(W/'cache/pycache'),MAMBA_ROOT_PREFIX='/home/footfalcon/miniforge3',
...
with (W/'executor.lock').open('a') as lock:
    ...
    p=subprocess.Popen(box+argv,cwd=W,env=env,stdout=subprocess.PIPE,stderr=subprocess.PIPE,start_new_session=True,preexec_fn=bounds);timed=False
    ...
    paths={}
    for k,data in [('stdout',out),('stderr',err)]:
        rel='logs/'+tag+'.'+k
    ...
    row=dict(start_utc=start,cwd=str(W),argv=argv,sandbox=box,exit_code=p.returncode,timeout=timed,
        wall_limit_seconds=limit,cpu_limit_seconds=limit,address_space_bytes=None if asan else 8*1024**3,
        asan_shadow_reservation=asan,elapsed_seconds=time.monotonic()-tick,**paths,
