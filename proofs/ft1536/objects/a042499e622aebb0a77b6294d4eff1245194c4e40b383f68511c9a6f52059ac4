...
def copy(p,r):
 ...
 if dst.exists():assert sha(dst)==sha(p);return
 shutil.copyfile(p,dst);rows.append(dict(input=p.relative_to(W).as_posix(),copy=r,sha256=sha(p),byte_identical=True))
for src,names in [('ORDERED',['ordered_model','root_model']),('POST',['post_model','codec_model']),('GAUSS',['scope']),('IID',[])]:
