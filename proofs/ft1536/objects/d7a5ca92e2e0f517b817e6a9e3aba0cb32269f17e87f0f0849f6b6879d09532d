...
r=dict(argv=cmd,cwd=str(W),exit_code=p.returncode,wall_limit=120,elapsed=time.monotonic()-t,stdout_sha256=sha(W/'logs/model_values.stdout'),stderr_sha256=sha(W/'logs/model_values.stderr'))
...
out=dict(status='PASS_SOURCE_NORMALIZED_MODEL_VALUES',pairs=len((W/'checks/model_jobs.txt').read_text().splitlines()),output_sha256=sha(W/'checks/model_values.txt'),
 operations=['mulC','divC'],domain='new div endpoints1/16 and2^35 included; finite controls, not a universal proof')
