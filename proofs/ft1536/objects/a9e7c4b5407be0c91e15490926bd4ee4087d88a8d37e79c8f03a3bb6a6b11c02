...
W=Path.cwd();jobs=[['/usr/bin/gcc','--version'],['/usr/bin/gcc','-dumpmachine'],['/home/footfalcon/.elan/toolchains/leanprover--lean4---v4.34.0/bin/lean','--version'],['/home/footfalcon/.local/bin/sage','--version'],['/usr/bin/python3','--version']]
texts=[];rows=[]
for i,cmd in enumerate(jobs):
 ...
 rows.append(dict(argv=cmd,cwd=str(W),exit_code=p.returncode,wall_limit=45,elapsed=time.monotonic()-t,stdout_sha256=sha(W/'logs'/f'toolchain_{i}.stdout'),stderr_sha256=sha(W/'logs'/f'toolchain_{i}.stderr')))
...
for p in [W/'source/falcon-sign.c',W/'inputs/bootstrap/MANIFEST.sha256']:
 try:fd=os.open(p,os.O_WRONLY);os.close(fd);raise AssertionError('source must be RO')
